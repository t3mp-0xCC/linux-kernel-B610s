...
#ifndef CONFIG_MMC_DW_UPDATE_BALONG
...
{
	host->cmd = cmd;
#ifdef CONFIG_MMC_DW_UPDATE_BALONG
	dev_dbg(host->dev,
...
#ifndef CONFIG_MMC_DW_UPDATE_BALONG
    host->timing != MMC_TIMING_MMC_HS400 && /* unsupported!! */
#endif
...
static void dw_mci_set_ios(struct mmc_host *mmc, struct mmc_ios *ios)
{
	...
	case MMC_POWER_OFF:
		...
		mci_writel(slot->host, PWREN, regs);
#ifdef CONFIG_MMC_DW_UPDATE_BALONG
		(void)dw_mci_switch_voltage(mmc, ios);
#endif
		break;
	default:
		break;
...
#endif
#ifdef CONFIG_MMC_DW_UPDATE_BALONG
static int dw_mci_setup_voltage(struct dw_mci_slot *slot, struct mmc_ios *ios)
{
	struct dw_mci *host = slot->host;
	const struct dw_mci_drv_data *drv_data = slot->host->drv_data;
	u32 clk_en_a;
	...
	/* inform CIU */
	mci_send_cmd(slot,
		     SDMMC_CMD_UPD_CLK | SDMMC_CMD_PRV_DAT_WAIT | SDMMC_CMD_VOLT_SWITCH, 0);
	/*switch to 1.8v*/
	if (drv_data && drv_data->set_ios)
		err = drv_data->switch_voltage(slot->mmc, ios);
	...
	struct mmc_ios *ios)
{
	...
	if (ios->signal_voltage == MMC_SIGNAL_VOLTAGE_330) {
		...
		if (drv_data && drv_data->set_ios)
			err = drv_data->switch_voltage(mmc, ios);

	} else if (ios->signal_voltage == MMC_SIGNAL_VOLTAGE_180) {
		err = dw_mci_setup_voltage(slot, ios);
...
static int dw_mci_switch_voltage(struct mmc_host *mmc, struct mmc_ios *ios)
{
    return dw_mci_start_signal_voltage_switch(mmc, ios);
}
#else
static int dw_mci_switch_voltage(struct mmc_host *mmc, struct mmc_ios *ios)
{
...
#ifdef CONFIG_MMC_DW_UPDATE_BALONG
		present = (host->card_detect & (1 << slot->id))
		    == 0 ? 1 : 0;
#else
...
int dw_mci_suspend(struct dw_mci *host)
{
	...
	for (i = 0; i < host->num_slots; i++) {
		...
		if (host->drv_data) {
			...
			if (!IS_ERR(slot->mmc->supply.vmmc)) {
				ret = mmc_regulator_set_ocr(slot->mmc, slot->mmc->supply.vmmc, 0);
				if (ret) {
					dev_err(slot->host->dev,
						"failed to enable vmmc regulator\n");
					return ret;
...
int dw_mci_resume(struct dw_mci *host)
{
	...
	for (i = 0; i < host->num_slots; i++) {
		...
		if (host->drv_data) {
			...
			if (!IS_ERR(slot->mmc->supply.vmmc)) {
				ret = mmc_regulator_set_ocr(slot->mmc, slot->mmc->supply.vmmc, ffs(MMC_VDD_28_29) - 1); // just for sd card.
				if (ret) {
					dev_err(slot->host->dev,
						"failed to enable vmmc regulator\n");
					return ret;
