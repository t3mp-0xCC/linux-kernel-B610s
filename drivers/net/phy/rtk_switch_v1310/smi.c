...
extern struct mutex rtk_mdio_lock;

extern void mbb_mdio_write(rtk_uint32 mAddrs, rtk_uint32 wData);
extern void mbb_mdio_read(rtk_uint32 mAddrs, rtk_uint32 *rData);
...
#else
    /* Write address control code to register 31 */
    mbb_mdio_write(MDC_MDIO_CTRL0_REG, MDC_MDIO_ADDR_OP);

    /* Write address to register 23 */
    mbb_mdio_write(MDC_MDIO_ADDRESS_REG, mAddrs);

    /* Write read control code to register 21 */
    mbb_mdio_write(MDC_MDIO_CTRL1_REG, MDC_MDIO_READ_OP);

    /* Read data from register 25 */
    mbb_mdio_read(MDC_MDIO_DATA_READ_REG, rData);
#endif
    /* Unlock */
...
rtk_int32 smi_write(rtk_uint32 mAddrs, rtk_uint32 rData)
{
    ...
    /* Write data to register 24 */
    mbb_mdio_write(MDC_MDIO_DATA_WRITE_REG, rData);

    /* Write data control code to register 21 */
    mbb_mdio_write(MDC_MDIO_CTRL1_REG, MDC_MDIO_WRITE_OP);
#endif
    /* Unlock */
    rtlglue_drvMutexUnlock();
