...
//extern static int FTS_IS_FW_UPLOADING;
/*******************************************************************************
...
static unsigned char CTPM_FW[] = {
    #include "FT_Upgrade_App.h"
};
#elif defined(BSP_CONFIG_BOARD_E5_DC04)
static unsigned char CTPM_FW[] = {
    #include "FT_Upgrade_App_DC04.h"
};
#endif
...
int ft6x36_read_VendorID(struct i2c_client * client, char * pVendorID) 
{
    u8 reg_val[2] = {0};
    u32 i = 0;
    u32 temp;
    ...
    for (i = 0; i < FTS_UPGRADE_LOOP; i++) 
    {
        ...
        {
            printk(KERN_ERR "[FTS] Step 3: GET CTPM ID FAIL,ID1 = 0x%x,ID2 = 0x%x\n", reg_val[0], reg_val[1]);
        }
    }
    ...
    fts_i2c_read(client, packet_buf, sizeof(packet_buf), pVendorID, 1);
...
int fts_ctpm_auto_upgrade(struct i2c_client *client)
{
    ...
    /*整机升级功能，包含正常升级和异常升级两种情况*/
    fts_read_reg(client, FTS_REG_ID, &uc_tp_fm_chipid);
    /*正常升级，此时代码区域没有损坏，可以直接从代码区域读到fw ver\vendor id\chip id等信息*/
    if(uc_tp_fm_chipid == 0x36)
    {
        /*获得现在FW的版本号*/
    ...
    /*异常升级，代码区域损坏，需要强制升级，通过从bootloader中读取vendor id来区分模组*/
    {
        /*read vendor id from bootloader*/
        i_ret = ft6x36_read_VendorID(client,  &VendorID);
        if (i_ret != 0)
        {
            dev_err(&client->dev,"%s:read vendor id from bootloader failed!\n", __func__);
            return -EIO;
        }

        /*此处从bootloader中读取vendorid*/
        if(VendorID == 0x51)
        {
            /*直接调用升级函数实现整机升级功能*/
            memcpy(&fts_updateinfo_curr, &fts_updateinfo[0], sizeof(struct fts_Upgrade_Info));
        ...
        {
            dev_err(&client->dev,"[FTS] the vendor id is wrong!.\n");
            return -EIO;
        }
    }
