...
int fixed_fs_partitions(struct mtd_info *master, void *partion)
{
    ...
    if ( MTD_YAFFS2_FS == fs_type || MTD_JIFFS2_FS == fs_type )
    {
        ...
        {
            /* 当pos块对齐,且小于mtd设备大小,且为坏块时继续扫描 */
            if((0 == (pos % part->mtd.erasesize))
                && (part->mtd.size > (uint64_t)pos)
            ...
            part->offset =  part->offset + (uint64_t)pos;
            /* 如果已超出范围,如分区均为坏块的场景则设置size为0 */
            part->mtd.size = (uint64_t)((part->mtd.size > (uint64_t)pos) ? (part->mtd.size - (uint64_t)pos) : 0);
