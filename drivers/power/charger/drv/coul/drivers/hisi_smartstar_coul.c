...
int v_offset_a = 1000000;   /* need div 1000000 */
int v_offset_b = 0;         /* uv */
int c_offset_a = 1000000;   /* need div 1000000 */
int c_offset_b = 0;         /* ua */
...
#define BATT_CALI_MIN_VOLT  (3450)/*电压校准门限*/
#define BATT_CALI_MAX_VOLT  (4350)/*电压校准门限*/
#else
#define BATT_CALI_MIN_VOLT  (3400)/*电压校准门限*/
#define BATT_CALI_MAX_VOLT  (4200)/*电压校准门限*/
...
#define FIFO_DEPTH    (10)/*初始数据可能有误，丢弃最初5组数据*/
...
extern void coul_reg_write(u16 reg, u8 val);
extern u8 coul_reg_read(u16 reg);
...
/*Hi6421 Reg CLJ_CTRL_REG, 0x151: ECO filter timer如下设定分别对应100-400ms.*/
...
#define SMARTSTAR_COUL_IRQ_REG      0x14b
#define SMARTSTAR_COUL_IRQ_MASK_REG 0x14c

#define COUL_CALI_ING     (0x03)

#define SMARTSTAR_COUL_CTRL_REG 0x151
#define SMARTSTAR_CL_OUT_BASE   0x153
#define SMARTSTAR_CL_IN_BASE    0x157
#define SMARTSTAR_CHG_TIMER_BASE 0x15B
#define SMARTSTAR_LOAD_TIMER_BASE 0x15F
#define SMARTSTAR_CL_INT_BASE 0x163
#define SMARTSTAR_VOL_INT_BASE    0x167
#define SMARTSTAR_OFFSET_CURRENT 0x169
#define SMARTSTAR_OFFSET_VOLTAGE 0x16B
#define SMARTSTAR_OCV_REG_BASE  0x16D
#define SMARTSTAR_VOL_FIFO_BASE 0x179
#define SMARTSTAR_CUR_FIFO_BASE 0x18D
#define SMARTSTAR_COUL_STATE_REG 0x1AF
/*ocv and ocv flag take 2bytes, so 0x1a6:0x1a5 used.*/
#define SMARTSTAR_SAVE_OCV_ADDR 0x1A5
#define SMARTSTAR_FIFO_CLEAR 0x1AE        /*Reg 0x1AE Bit3 on Hi6421*/
...
#define SMARTSTAR_DEBUG_REG 0x1AD
...
{
    /*获取待返回字符串的真实长度*/
    int len = snprintf(NULL, 0, "%d", do_save_offset_ret);

    if (buffer)
    {
        return snprintf(buffer, len, "%d", do_save_offset_ret);
    }
    else
    {
        return -EINVAL;
...
static int batt_voltage_uv_ops_get(char *buffer, const struct kernel_param *kp)
{
    ...
    /*获取待返回字符串的真实长度*/
    int len = snprintf(NULL, 0, "%d", vol);

    if (buffer)
    {
        return snprintf(buffer, len, "%d", vol);
    }
    else
    {
        return -EINVAL;
...
static int convert_regval2uv_nocail(short reg_val);
...
static int batt_init_ocv_ops_get(char *buffer, const struct kernel_param *kp)
{
    ...
    int ocv             = 0;
    /*获取待返回字符串的真实长度*/
    int len = snprintf(NULL, 0, "%d uv", ocv);

    /* read ocv and offset */
    SMARTSTAR_REGS_READ(SMARTSTAR_OCV_REG_BASE, &ocvreg, 2);
    SMARTSTAR_REGS_READ(SMARTSTAR_OFFSET_VOLTAGE, &volreg_offset, 2);
    ocv = convert_regval2uv_nocail(ocvreg - volreg_offset);

    if (buffer)
    {
        return snprintf(buffer, len, "%d uv", ocv);
    }
    else
    {
        return -EINVAL;
...
{
    /*获取待返回字符串的真实长度*/
    int len = snprintf(NULL, 0, "%d", coul_running);

    if (buffer)
    {
        return snprintf(buffer, len, "%d", coul_running);
    }
    else
    {
        return -EINVAL;
                                        ...
                                        const struct kernel_param *kp)
{
    ...
    {
        /*获取待返回字符串的真实长度*/
        len = snprintf(NULL, 0, "%d", di->batt_soc_with_uuc);
        return snprintf(buffer, len, "%d", di->batt_soc_with_uuc);
    }
    else
    {
        return -EINVAL;
...
static int  pmussi_recv_data (int reg_addr, char *buffer, int len)
{
    ...
    for (i = 0; i < len; i++) {
        *(buffer + i ) = coul_reg_read(reg_addr + i);
        PMUSSI_DELAY();
    }
...
static int  pmussi_send_data (int reg_addr, char *buffer, int len)
{
    ...
    for (i = 0; i < len; i++) {
        coul_reg_write((reg_addr + i), *(buffer + i));
        PMUSSI_DELAY();
    }
...
static long calculate_cc_uah(void)
{
    ...
    SMARTSTAR_COUL_INF("cc_in = %lduAh, cc_out = %lduAh.\n", cc_uah_in, cc_uah_out);
...
static short convert_uv2regval_nocail(int uv_val)
{
    short ret;
    s64 temp;
...
/********************************************************
*函数名   : update_ioffset_value
*函数功能 : 内部接口，主要封装了根据产品名称更新电流校准参数的动作
*输入参数 : ss_coul_nv_info 指针
*输出参数 : 无
*返回值   : 无
*修改历史 :
*           2015-10-20 韩久平 初版作成
********************************************************/
void update_ioffset_value(struct ss_coul_nv_info *pinfo)
{
    unsigned int product_type = mbb_version_get_board_type();

    SMARTSTAR_COUL_INF("%s for product 0x%x.\n", __func__, product_type);

    if (unlikely(NULL == pinfo))
    {
        SMARTSTAR_COUL_ERR("%s pinfo is NULL.\n", __func__);
        return;
    }
    ...
    switch(product_type) {
        ...
        {
            /*SB03 V2单板，硬件提供的等效电阻值是10.31463mohm.*/
            pinfo->c_offset_a = 969496;
        }
        break;
        case HW_VER_PRODUCT_DC04:
        {
            /*DC04 单板，硬件提供的等效电阻值是10.30459792mohm.*/
            pinfo->c_offset_a = 971770;
        }
        break;
        default:
        break;
...
/*****************************重要********************************/
/*在烧片版本中尽量的不要去执行该函数中写v_offset_a和v_offset_b的操作*/
/*以防止将未校准的值写进去，目前产品中烧片版本不加载库仑计和充放电模块*/
/*应该不存在问题                                                  */
/*****************************************************************/
static int read_nv_info(struct smartstar_coul_device *di)
{
    ...
    struct ss_coul_nv_info sram_ss_coul_info;
    struct single_row_lut *preal_fcc_lut = &di->adjusted_fcc_temp_lut_tbl1;
    memset((void *)&sram_ss_coul_info, 0, sizeof(struct ss_coul_nv_info)); /*lint !e160 !e506 !e522*/
    memcpy(&sram_ss_coul_info, (unsigned int *)SRAM_COUL_ADDR, sizeof(struct ss_coul_nv_info));

    ret = bsp_nvm_read(NV_HUAWEI_COUL_INFO_I, 
                       (unsigned char *)pinfo, sizeof(struct ss_coul_nv_info));
    if (ret)
    {
        SMARTSTAR_COUL_ERR("read NV_HUAWEI_COUL_INFO_I error \n");
    }
    if((-1 == pinfo->v_offset_a) && (-1 == pinfo->v_offset_b) \
        && (-1 == pinfo->c_offset_a) && (-1 == pinfo->c_offset_b))
    {
         SMARTSTAR_COUL_ERR("NV_HUAWEI_COUL_INFO_I data error,reset!!!!\n");
         memset((void *)pinfo, 0, sizeof(struct ss_coul_nv_info));   /*lint !e160 !e506 !e522*/
    }

    /*由于50462是备份恢复项，所以这里需要手动更新一下校准值，避免使用旧值。*/
    update_ioffset_value(pinfo);
    ...
    if ((0 == pinfo->v_offset_a) || (0 == pinfo->v_offset_b))
    {
        ...
        if(!ret)
        {
            ...
            pinfo->v_offset_b = (BATT_CALI_MIN_VOLT 
                - vbatt_calib_value.min_value * (pinfo->v_offset_a / 1000)/1000) * 1000;
        }
    }

    pinfo->calc_ocv_reg_v = sram_ss_coul_info.calc_ocv_reg_v;
    pinfo->calc_ocv_reg_c = sram_ss_coul_info.calc_ocv_reg_c;
    pinfo->start_cc = 
      sram_ss_coul_info.start_cc == -1 ? pinfo->start_cc : sram_ss_coul_info.start_cc;
    pinfo->hkadc_batt_temp = sram_ss_coul_info.hkadc_batt_temp;
    pinfo->hkadc_batt_id_voltage = sram_ss_coul_info.hkadc_batt_id_voltage;
    pinfo->ocv_temp = 
      sram_ss_coul_info.ocv_temp == -99 ? pinfo->ocv_temp : sram_ss_coul_info.ocv_temp;
    ...
    SMARTSTAR_COUL_INF("v_a=%d,v_b=%d,c_a=%d,c_b=%d,cycles=%d,r_pcb=%d, reg_c=%d, "\
        "reg_v=%d, start_cc=%d,ocv_temp=%d\n", pinfo->v_offset_a, pinfo->v_offset_b, 
        pinfo->c_offset_a, pinfo->c_offset_b, pinfo->charge_cycles, pinfo->r_pcb, 
        pinfo->calc_ocv_reg_c, pinfo->calc_ocv_reg_v, pinfo->start_cc,pinfo->ocv_temp);
...
static int read_nv_info_real(struct smartstar_coul_device *di)
{
    ...
    SMARTSTAR_COUL_INF("v_a=%d,v_b=%d,c_a=%d,c_b=%d,cycles=%d,r_pcb=%d, reg_c=%d, "\
        ...
        pinfo->c_offset_b, pinfo->charge_cycles, pinfo->r_pcb, pinfo->calc_ocv_reg_c, 
        pinfo->calc_ocv_reg_v, pinfo->start_cc);
...
static int save_nv_info(struct smartstar_coul_device *di)
{
    ...
    if (!di->is_nv_read){
        ...
        SMARTSTAR_REG_WRITE(SMARTSTAR_FIFO_CLEAR, val & 0xf7);
 ...
 * 2015.12.08: 重新实现此函数，用于获取FIFO中电流值的平均值。
 */
int smartstar_battery_current_avg(void)
{
#if defined(SMARTSTAR_GET_IBAT_AVG_WITHIN_2MIN)
    struct smartstar_coul_device *di = g_smartstar_coul_dev;
    return di->last_iavg_ma;
#else
    short regval       = 0;
    int32_t i          = 0, used = 0;
    int32_t current_ua = 0, totalcur = 0;

    for (i = 0; i < FIFO_DEPTH; ++i)
    {
        /*Each current value take 2 bytes.*/
...
{
    short ocvreg = convert_uv2regval_nocail(ocv);
 ...
 */
static void get_ocv_by_soc(struct smartstar_coul_device *di,int batt_temp,int soc)
{
    ...
    new_ocv = interpolate_ocv(di->batt_data->pc_temp_ocv_lut, batt_temp_degc, soc);
...
static void get_ocv_by_vol(struct smartstar_coul_device *di)
{
    ...
    for (i = 0; i < 10; i++)
    {
            ...
            || current_ua < CHARGING_CURRENT_OFFSET)
        {
            ...
            SMARTSTAR_COUL_INF("[%d]invalid regval = %d current = %d ua\n",i,regval,current_ua);
            continue;
        }
        ...
        {
            DBG_CNT_INC(dbg_invalid_vol);
            SMARTSTAR_COUL_INF("[%d]invalid regval = %d voltage = %d uv\n",i,regval, voltage_uv);
            continue;
        }
        SMARTSTAR_COUL_INF("valid voltage = %d uv\n", voltage_uv);
        totalvol += voltage_uv;
              totalcur += current_ua;
        used++;
    }
...
static void get_initial_ocv(struct smartstar_coul_device *di)
{
    ...
    if (ocvreg == (unsigned short)FLAG_USE_CLAC_OCV){
        ocv = get_calc_ocv(di);
        /*判断是否是过放引起的,不是过放不进行自学习*/
        if (ocv > 3200000)
        {
            di->batt_ocv_valid_to_refresh_fcc = 0;
        }
        ocvreg = convert_uv2regval_nocail(ocv);
        ...
        ocv = convert_regval2uv_nocail(ocvreg);
        SMARTSTAR_REGS_WRITE(SMARTSTAR_SAVE_OCV_ADDR,&ocvreg,2);
    }
    else{
        if (ocvreg & 0x8000){
    ...
    if(fastboot_save_nv_info_error()){
        ...
        if(ocv < (int)(di->v_cutoff * 1000)) {
            SMARTSTAR_COUL_INF("ocv = %d is too low, ", ocv);
            /*使ocv略大于建模0点电压值，使得初始ocv<3.2v时也能进行自学习*/
            ocv = (int)(di->v_cutoff * 1000 + 1);
            SMARTSTAR_COUL_INF("adjust ocv to %d\n", ocv);
        }
    }

    di->batt_limit_fcc = pinfo->limit_fcc;

    SMARTSTAR_COUL_INF("ocv = %d uv, start_cc = %ld uAh, ocv_temp =%d\n", 
                        ocv, di->cc_start_value,pinfo->ocv_temp);
...
#define BATTERY_VOL_2_PERCENT    (3250)/*定义低电关机门限，用于低电中断上报*/
#define BATTERY_VOL_CUTOFF    (3050)/*定义放电截止电压门限*/
#else
...
static void get_fifo_data(struct vcdata *vc)
{
    ...
    for (i=0; i<FIFO_DEPTH; i++){
        ...
        {
            SMARTSTAR_COUL_INF("[%d]invalid regval = %d voltage = %d mv\n",i,vol_fifo[i],vol);
            SMARTSTAR_COUL_INF("[%d]invalid regval = %d current = %d ma\n",i,cur_fifo[i],cur);
            continue;    
        }
        vols += vol;
        curs += cur;
...
static int limit_soc(struct smartstar_coul_device *di,int input_soc)
{
    ...
    SMARTSTAR_COUL_INF("limit soc input soc = %d\n", input_soc);
    SMARTSTAR_COUL_INF("current_ua=%d voltage_uv=%d.\n", current_ua, voltage_uv);
...
#if defined(BSP_CONFIG_BOARD_E5_DC04)
    /*如果当前电池电压满足电量向下平滑条件，则迅速将soc向下平滑*/
    if (chg_is_batt_power_collapse() && (last_soc > (BATT_CAPACITY_SHUTOFF + 1)))
    {
        output_soc = last_soc - 1;
        SMARTSTAR_COUL_ERR("DC04 in power collapse, adjust to %d Percents\n", output_soc);
    }
#endif
    /*电池达到低电关机门限，迅速将soc平滑至关机门限*/
    if(chg_is_batt_in_state_of_emergency() && (last_soc > BATT_CAPACITY_SHUTOFF))//0%低电关机
    {
        output_soc = last_soc - 1;
        SMARTSTAR_COUL_ERR("emergency!!adjust Battery Capacity to %d Percents\n", output_soc);
    }
    /*电压未达到关机门限时，soc低于关机门限2%,则显示3%*/
    if ((output_soc <= BATT_CAPACITY_SHUTOFF) && (!chg_is_batt_in_state_of_emergency())) {
        output_soc = BATT_CAPACITY_SHUTOFF + 1;
    ...
    if (di->charging_state == CHARGING_STATE_CHARGE_DONE){
        ...
        /*修改魔鬼数字，使用宏关联充放电复充门限，区分高温充电OCV刷新情况下的复充门限为满电真实电量减满电电量与复充门限差值*/
        if ( BATT_CAPACITY_RECHG <= input_soc )
        {
            if ( BATT_CAPACITY_RECHG == input_soc )
            {
                di->charging_state = CHARGING_STATE_CHARGE_NOT_CHARGE;
                SMARTSTAR_COUL_INF("when recharging that the batt recharge is %d!\n", BATT_CAPACITY_RECHG);
            }
        }
        else
        {
            if ( di->batt_recharge_thr == input_soc )
            {
                di->charging_state = CHARGING_STATE_CHARGE_NOT_CHARGE;
                SMARTSTAR_COUL_INF("when recharging that the batt recharge is %d!\n", di->batt_recharge_thr);
            }
        }
    }
...
/*******************************************************************
Function:      fresh_temp_of_battery
Description:   刷新库仑计中记录的电池温度
Data Accessed: 无
Data Updated:  无
Input:         di : 库仑计状态全局变量记录值
Return:        无
*******************************************************************/
static void fresh_temp_of_battery(struct smartstar_coul_device *di)
{
    int temp = 0;
...
#if (MBB_CHG_PLATFORM_BALONG == FEATURE_ON)
/********************************重要重要重要*******************************************
  calculate_state_of_charge函数在计算电量之前会刷新电池温度，在V7R2平台上查询温度需要调用核间通信
  核间通信会导致DI_LOCK锁时间大幅度增加，从而导致概率性死锁，单板重启，因此在V7R2平台刷新电池温度在该
  函数之外进行，以便能够将核间通信放到DI_LOCK锁之外
*********************************重要重要重要********************************************/
#else
    int temp;
    ...
    SMARTSTAR_COUL_INF("FCC=%duAh, UUC=%duAh, RC=%duAh, CC=%duAh, delta_RC=%duAh, Rbatt=%dmOhm\n",
                       fcc_uah, unusable_charge_uah, remaining_charge_uah, cc_uah, 
                       delta_rc_uah, rbatt);
    ...
    soc= adjust_soc(di, soc, di->batt_temp, di->batt_chargecycles/100, rbatt, 
...
/*检测到电池拔出，清除寄存器信息*/
void smartstar_battery_plug_out(void)
{
    short reg = 0;
    struct smartstar_coul_device *di = g_smartstar_coul_dev;

    SMARTSTAR_COUL_ERR("%s: Enter\n",__FUNCTION__);

    di->batt_exist = 0;
    cancel_delayed_work(&di->calculate_soc_delayed_work); /*lint !e534*/
    SMARTSTAR_REG_WRITE(SMARTSTAR_BATTERY_MOVE_ADDR, 0x18);/*lint !e534*/
    /*clear saved ocv*/
    SMARTSTAR_REG_WRITE(SMARTSTAR_SAVE_OCV_ADDR, reg);/*lint !e534*/

    SMARTSTAR_COUL_ERR("%s: Exit\n",__FUNCTION__);
}
...
static void calculate_soc_work(struct work_struct *work)
{
...
#if (MBB_CHG_PLATFORM_BALONG == FEATURE_ON)
    fresh_temp_of_battery(di);
#endif
    cali_adc_enable = cali_cnt % (CALIBRATE_INTERVAL / di->soc_work_interval);
    ...
    DI_UNLOCK();
    if (di->charging_state == CHARGING_STATE_CHARGE_DONE
        /*充电状态机在满电态时也允许进行电量修正，解决合入防鼓胀方案后，
          因库仑计电量采集精度问题，导致的电量持续下降问题。*/
        || CHG_STM_MAINT_ST == chg_stm_get_cur_state()){
        if(charged_cnt == 0){
...
static void interrupt_notifier_work(struct work_struct *work)
{
    ...
    if (intstat & COUL_VBAT_LOW_INT)
    {
        ...
        {
            SMARTSTAR_COUL_ERR("IRQ: COUL_VBAT_LOW_INT, no battery, error\n");
            return;
        }
...
static COUL_INT_FUNC coul_irq_cb[IRQ_MAX] =
{
    (COUL_INT_FUNC)coul_irq_cb_COUL_INT,
    (COUL_INT_FUNC)coul_irq_cb_COUL_OUT,  
    (COUL_INT_FUNC)coul_irq_cb_COUL_IN,
    (COUL_INT_FUNC)coul_irq_cb_VBAT_INT
};
                                ...
                                int *ret_fcc_uah)
{
    ...
    *ret_fcc_uah = fcc_uah;
    SMARTSTAR_COUL_INF("real_fcc=%d, RC=%d CC=%d fcc=%d charging_begin_soc=%d.%d\n",
                       ...
                       di->charging_begin_soc/10, di->charging_begin_soc);
    return real_fcc_uah;
}
...
static void readjust_fcc_table(struct smartstar_coul_device *di)
{
    ...
    {
        temp->x[i] = now->x[i];
        ratio = div_u64((unsigned long long)(now->y[i]) * 1000, fcc);
...
static void smartstar_charging_done (struct smartstar_coul_device *di)
{
    ...
    refresh_fcc(di);
    /*更新 复充门限 = 满电时真实电量 - (满电电量与复充门限的差值)*/
    di->batt_recharge_thr = di->batt_soc - (BATT_CAPACITY_FULL - BATT_CAPACITY_RECHG);
    SMARTSTAR_COUL_INF("The full soc is %d, recharge threshold %d\n",
                    di->batt_soc, di->batt_recharge_thr);
    ...
    /*实际上是通过SOC进行反查表得到当前OCV的值，对改接口进行优化重构*/
    get_ocv_by_soc(di, di->batt_temp,di->batt_soc);
...
void ss_time_show(void)
{
    ...
    printk(KERN_ERR"cl_in_time=%d,cl_out_time = %d,total = %d\n",\
            (unsigned int)cl_in_time,(unsigned int)cl_out_time,
...
{
    int uf_temp = 0, temp = 0, voltage = 0, ufcapacity = 0, capacity = 100;
    int afcapacity = 0, rm = 0, fcc = 0, delta_rc = 0, len = 0;
    ...
    {
        /*获取需要打印的字符串长度*/
        len = snprintf(NULL, 0,   "%s", "Smartstar coulometer probe failed!\n");
        return snprintf(buf, len, "%s", "Smartstar coulometer probe failed!\n"); 
    }
    else
    {
        //do nothing
    ...
    /*获取需要打印的字符串长度*/
    len = snprintf(NULL, 0, "%-6d  %-6d  %-8d  %-6d  %-3d  %-5d  %-6d  "\
        "%-6d  %-5d  %-6d  %-5d  %-4d  %-7d  %-5d  %-5d  ", voltage,  (signed short)cur, 
        ufcapacity, capacity, afcapacity, rm, fcc, uuc, cc, delta_rc, uf_temp, temp, 
        ocv, rbatt, di->batt_limit_fcc/1000);  
    return snprintf(buf, len, "%-6d  %-6d  %-8d  %-6d  %-3d  %-5d  %-6d  "\
        "%-6d  %-5d  %-6d  %-5d  %-4d  %-7d  %-5d  %-5d  ", voltage,  (signed short)cur, 
        ufcapacity, capacity, afcapacity, rm, fcc, uuc, cc, delta_rc, uf_temp, temp, 
        ocv, rbatt, di->batt_limit_fcc/1000);
}

static ssize_t smartstar_set_hand_chg_capacity_flag(struct device_driver *driver, 
...
#else
static int __init hisi_smartstar_coul_probe(struct platform_device *pdev)/*lint !e10*/
#endif
{/*lint !e10 !e64*/
    ...
    do {
        /*Hi6421 Reg[0x04] Version: 0x05, Reg[0x05] Chip_id: 0x20*/
        retval  = SMARTSTAR_REG_READ(0x04) << 8;
        retval |= SMARTSTAR_REG_READ(0x05);

        SMARTSTAR_COUL_INF("do a dummy read, smartstar version is V%x\n",
                            retval);
    ...
    /*Hisi UDP use V510 Hi6421*/
    } while(retval != 0x0520 && retval != 0x0510);
    ...
    retval = driver_create_file(&(hisi_smartstar_coul_driver.driver), &driver_attr_gaugelog);
        if (0 != retval) {
            ...
            return -1;/*lint !e110 !e82 !e533*/
        }
    retval = driver_create_file(&(hisi_smartstar_coul_driver.driver), 
                                &driver_attr_hand_chg_capacity_flag);
        if (0 != retval) {
            printk("failed to create sysfs entry(hand_chg_capacity_flag): %d\n", retval);
            return -1;/*lint !e110 !e82 !e533*/
        }
    retval = driver_create_file(&(hisi_smartstar_coul_driver.driver), 
                                &driver_attr_input_capacity);
        if (0 != retval) {
            printk("failed to create sysfs entry(input_capacity): %d\n", retval);
            return -1;/*lint !e82 !e110 !e533*/
        }
    ...
    di->is_nv_need_save = 0;
    /*复充门限初始化*/
    di->batt_recharge_thr = BATT_CAPACITY_RECHG;
...
coul_no_battery:
    ...
    coul_ops->battery_removal_handle = smartstar_battery_plug_out;
    ...
    return retval;/*lint !e110 !e82 !e533*/
...
coul_failed_1:
    ...
    SMARTSTAR_COUL_ERR("Smartstar probe failed!\n");
    return retval;/*lint !e110 !e82 !e533*/
coul_failed_0:
    smartstar_disable_coul(di);
coul_failed:
...
static int __exit hisi_smartstar_coul_remove(struct platform_device *pdev)
{
    struct smartstar_coul_device *di = platform_get_drvdata(pdev);

    if(NULL == di)
    {
        SMARTSTAR_COUL_ERR("di is NULL!\n");
        return (0 - EIO);
    }
    ...
    pm_message_t state)
{
    ...
    if (wakeup_time > SR_MAX_RESUME_TIME) {
        ...
        SMARTSTAR_COUL_ERR("[SR]%s(%d): wakeup_time=%ld, di->sr_suspend_time=%ld, "\
            "di->sr_resume_time=%ld\n", __func__, __LINE__, wakeup_time, 
...
static int sr_need_update_ocv(struct smartstar_coul_device *di) {
    ...
    if (sr_index_wakeup >= 0 && sr_index_wakeup < SR_ARR_LEN) {
        last_wakeup_time = (sr_index_wakeup - 1 < 0) ? \
            sr_time_wakeup[SR_ARR_LEN - 1]: sr_time_wakeup[sr_index_wakeup - 1];
    }

    /* get last sleep time */
    if (sr_index_sleep >= 0 && sr_index_sleep < SR_ARR_LEN) {
        last_sleep_time = (sr_index_sleep - 1 < 0) ? \
            sr_time_sleep[SR_ARR_LEN - 1]: sr_time_sleep[sr_index_sleep - 1];
    }
    ...
    if (last_sleep_time > SR_DELTA_SLEEP_TIME &&
        ...
        SMARTSTAR_COUL_INF("[SR]%s(%d): need_update, last_sleep=%lds, last_wakeup=%lds, "\
            "duty_ratio=%ld\n", __func__, __LINE__, last_sleep_time, last_wakeup_time, duty_ratio);
        return 1;
    }
    else {
        SMARTSTAR_COUL_INF("[SR]%s(%d): no_need_update, last_sleep=%lds, last_wakeup=%lds, "\
            "duty_ratio=%ld\n", __func__, __LINE__, last_sleep_time, last_wakeup_time, duty_ratio);
        return 0;
    }
}
...
static int hisi_smartstar_coul_resume(struct platform_device *pdev)
{
    ...
    if (ocv_could_update(di)){
        ...
        if (old_ocv != di->batt_ocv){
            short ocvreg = convert_uv2regval_nocail(di->batt_ocv);
            ocvreg = ocvreg | 0x8000;
            udelay(100);
            SMARTSTAR_REGS_WRITE(SMARTSTAR_SAVE_OCV_ADDR,&ocvreg,2);
            di->batt_ocv_valid_to_refresh_fcc = 0;
...
static struct platform_driver hisi_smartstar_coul_driver = {
    ...
    .remove     = __exit_p(hisi_smartstar_coul_remove),
#ifdef CONFIG_PM
...
module_init(hisi_smartstar_coul_init);
