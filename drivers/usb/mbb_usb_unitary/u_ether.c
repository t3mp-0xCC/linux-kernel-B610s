...
static unsigned int g_usb_portno = 0;
/*-------------------------------------------------------------------------*/
...
#if (defined(CONFIG_BALONG_SPE) || defined(CONFIG_BALONG_SPE_MODULE))
int eth_adp_for_spe_init(struct eth_dev *dev, USB_UINT32 cdc_type);
static int eth_adp_for_spe_exit(struct eth_dev *dev);
...
static void rx_spe_complete(struct usb_ep *ep, struct usb_request *req)
{
	...
	case 0:
		...
		while (skb2) {		
			...
			while (skb3 = skb_dequeue(&dev->spe_td_pending)) {
				dma = spe_hook.get_skb_dma(skb3);
				td_full = spe_hook.td_config(dev->spe_port_num, skb3, dma, spe_l3_bottom, 0);
				if (td_full) {
					skb_queue_head(&dev->spe_td_pending, skb3);
					break;
				}
			}
			if(!td_full){
				dma = spe_hook.get_skb_dma(skb2);
				td_full = spe_hook.td_config(dev->spe_port_num, skb2, dma, spe_l3_bottom, 0);
			}
			if (td_full) {
				skb_queue_tail(&dev->spe_td_pending, skb2);
			}
			skb2 = skb_dequeue(&dev->rx_frames);
		}

        if(dev->ctx.cdc_bypass && !(gnet_dbg_level & GNET_DBG_RX2ADS)){
            defer_rx(dev);
        }
...
#ifdef CONFIG_BALONG_SKB_MEMBER
    if (skb && GNET_MAGIC_SKB_USED == skb->used_check) {
...
static USB_VOID eth_start(struct eth_dev *dev, gfp_t gfp_flags, int is_resume)
{
    //DBG_T(MBB_NET, "%s\n", __func__);
...
#if (defined(CONFIG_BALONG_SPE) || defined(CONFIG_BALONG_SPE_MODULE))
    if (0 == spe_hook.port_is_enable(g_usb_portno))
    {
...
static USB_INT eth_stop(struct net_device *net)
{
    ...
    if (dev->port_usb) {
        ...
        const struct usb_endpoint_descriptor *in = link->in_ep->desc;;
        const struct usb_endpoint_descriptor *out = link->out_ep->desc;;
...
static USB_INT32 get_ether_addr(const USB_CHAR *str, USB_UINT8 *dev_addr)
{
    ...
    /*rand srand随机函数排查 删除*/
    DBG_T(MBB_NCM, "not support dynamic mac addr\n");
    return 1;
}
...
#if (defined(CONFIG_BALONG_SPE) || defined(CONFIG_BALONG_SPE_MODULE))
    GNET_SHOW_PRINT("| |--stat->tx_queue              :%lu\n",dev->ctx.stats.tx_queue);
    GNET_SHOW_PRINT("| |--stat->tx_spe_total      :%lu\n",dev->ctx.stats.tx_spe_total);
    GNET_SHOW_PRINT("| |--stat->tx_spe_complete_fail  :%lu\n",dev->ctx.stats.tx_spe_complete_fail);
    GNET_SHOW_PRINT("| |--stat->tx_spe_complete_ok  :%lu\n",dev->ctx.stats.tx_spe_complete_ok);
    GNET_SHOW_PRINT("| |--stat->tx_spe_rd_config    :%lu\n",dev->ctx.stats.tx_spe_rd_config);
    GNET_SHOW_PRINT("| |--stat->tx_packet_from_host :%lu\n",dev->ctx.stats.tx_packet_from_host);
    GNET_SHOW_PRINT("| |--stat->tx_spe_no_req     :%lu\n",dev->ctx.stats.tx_spe_no_req);
    GNET_SHOW_PRINT("| |--stat->tx_spe_send     :%lu\n",dev->ctx.stats.tx_spe_send);
#endif
                    ...
                    const USB_CHAR *buf, size_t size)
{
    ...
    /* coverity[secure_coding] */
        (void)sscanf(buf, "%s %lx",cmd_str,&cmd_value);/* [false alarm]:fortify disable */
    }
    else if(1 == g_gnet_cmdtbl[loop].format)
    {
    /* coverity[secure_coding] */
        (void)sscanf(buf, "%s %ld",cmd_str,&cmd_value);/* [false alarm]:fortify disable */
    }
    else
    {
        ;
...
static void gnet_send_spe_pending_td(struct eth_dev*	dev)
{
    struct sk_buff* skb;
    dma_addr_t dma;
    struct net_device* net = dev->net;
    ...
    while (NULL != (skb = skb_dequeue(&dev->spe_td_pending)))
    {
        dma = spe_hook.get_skb_dma(skb);
        ret = spe_hook.td_config(net->spe_portno, skb, dma, spe_l3_bottom, 0);
        if (ret == SPE_ERR_TDFULL)
        {
            //fixme:add debug count here.
            GNET_DBG(("%s: td full\n", __func__));
            break;
        }
    }
...
{
    unsigned int idx = geth_idx - 1;
    struct eth_dev* dev = the_dev[idx];

    gnet_recycle_rx_skb(dev, skb);
    gnet_send_spe_pending_td(dev);
...
int eth_adp_for_spe_init(struct eth_dev* dev, USB_UINT32 cdc_type)
{
    USB_INT ret = 0;
    USB_INT port_num = 0;
    USB_INT format = GNET_NTB32_FORMAT;
    ...
    ncm_spe_port_attr.rd_depth = MBB_SPE_USB_RD_TD_DEPTH;

    ncm_spe_port_attr.td_depth = MBB_SPE_USB_RD_TD_DEPTH;
    ...
    if (NULL == dev)
    {
        ...
        {
            DBG_E(MBB_NET, "%s can not alloc spe port for type %u\n", __FUNCTION__, cdc_type);
            return ( - EINVAL);
        }
    }
    else
    {
        ...
        if ((GNET_CDC_TYPE_NCM == dev->ctx.cdc_type)
            && (dev->get_wrap_format && dev->port_usb))
        {
        ...
        ncm_spe_port_attr.stick_mode = dev->ctx.cdc_bypass;
        ...
        switch (format)
        {
            ...
            case GNET_NTB16_FORMAT:
                ncm_spe_port_attr.enc_type = spe_enc_ncm_ntb16;
                break;
    ...
    if ((SPE_PORT_NUM < port_num ) || (0 > port_num))
    {
    ...
    {
        dev->spe_port_num = port_num;
        skb_queue_head_init(&dev->spe_td_pending);
        ret = spe_hook.port_enable(port_num);/*check ret*/

        DBG_I(MBB_NCM, "spe_hook.port_enable ret %d\n", ret);
    }
    return 0;
}

void bypass_spe_open(u32 dev_id)
{
    struct eth_dev* dev = the_dev[dev_id];

    if (0 == spe_hook.port_is_enable(g_usb_portno))
    {
        (void)spe_hook.port_enable(g_usb_portno);
    }

}
void bypass_spe_close(u32 dev_id)
{
    struct eth_dev* dev = the_dev[dev_id];
...
int gnet_spe_usb_plug(struct eth_dev* dev, unsigned int portno)
{
    ...
    int enc_type = spe_enc_ncm_ntb32;
    int ret = 0;
    unsigned int idx = geth_idx - 1;
    ...
    if (SPE_PORT_NUM <= portno)
    {
        GNET_ERR(("gnet_spe_usb_plug input port num(%u) err \n", portno));
        return ( - EINVAL);
    }
    port_property.enc_type = spe_enc_ncm_ntb32;
    port_property.desc_ops.finish_td = gnet_finish_spe_td;
    port_property.desc_ops.finish_rd = gnet_finish_spe_rd;
    port_property.desc_ops.finish_rd_bypass = NULL;
    ...
    /* config mac addr */
    if ((GNET_CDC_TYPE_NCM == dev->ctx.cdc_type)
        && (NULL != dev->get_wrap_format) && (NULL != dev->port_usb))
    {
        format = dev->get_wrap_format(dev->port_usb);
    }
    else if (GNET_CDC_TYPE_RNDIS == dev->ctx.cdc_type)
    ...
    if (0 != ret)
    {
        GNET_ERR(("Spe port enable error. \n"));
        return ret;
    }
 ...
 */
struct eth_dev * gether_setup(struct usb_gadget *g, USB_UINT8 ethaddr[ETH_ALEN])
{
 ...
 * Returns negative errno, or zero on success
 */
struct eth_dev *gether_setup_name(struct usb_gadget *g, USB_UINT8 ethaddr[ETH_ALEN],
        const USB_CHAR *netname)
{
    ...
    if (the_dev[geth_idx])
    {
         DBG_E(MBB_NET,"the_dev[geth_idx] :%p,geth_idx:%u\n",the_dev[geth_idx],geth_idx);
    ...
    if (!net)
    {
        DBG_E(MBB_NET,"call alloc_etherdev() fail\n");
    ...
    {
        GNET_INFO(("using default %s ethernet address\n", "self"));
    }
...
{
    struct eth_dev        *dev = link->ioport; //the_dev[link->port_no];
    USB_INT32           result = 0;
...
#if (defined(CONFIG_BALONG_SPE) || defined(CONFIG_BALONG_SPE_MODULE))
        if((0 != spe_hook.is_enable) && (0 != spe_hook.is_enable()))
        {
...
static USB_INT usb_prealloc_eth_rx_mem(struct eth_dev *dev, USB_UINT32 cdc_type_hint)
{
    ...
    DBG_T(MBB_NET, "%s : %lu, %lu, %p\n", __FUNCTION__, cdc_type, set_cdc_type, g_rx_skb_free_q);
...
USB_INT32 ncm_eth_mem_dump(USB_CHAR *buffer, USB_UINT32 buf_size)
{
    ...
    /* no room left */
    if (need_size + USB_NUM_8 > buf_size) {
        return -1;
    }
    cur += USB_NUM_2 * sizeof(USB_UINT32);
    ...
    *((int*)cur) = sizeof(struct eth_dev_ctx);
    ...
    return (USB_INT32)(need_size + USB_NUM_8);
}
