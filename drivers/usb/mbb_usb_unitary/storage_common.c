...
static int fsg_lun_open(struct fsg_lun *curlun, const char *filename)
{
	...
	num_sectors = (size + ((1 << curlun->blkbits) - 1)) >> curlun->blkbits; /* File size in logic-block-size blocks */
	min_sectors = 1;
	if (curlun->cdrom) {
			      ...
			      const char *buf, size_t count)
{
    ...
    DBG_T(MBB_MASS, "fsg_store_file!\n");
    if(pnp_api_handler->pnp_if_cdrom_can_open_cb)
    {
        if (MBB_USB_FALSE == pnp_api_handler->pnp_if_cdrom_can_open_cb())//如果是假关机或者关机状态，直接返回
        {
            DBG_T(MBB_MASS, "this mode do not let mass open!\n");
            return -EBUSY;
         }
    }
