...
struct f_ncm {
    ...
    bool is_inited_speed;
    bool is_notify_complete;
    u32 disep_at_req_incomp;
    u32 notify_noreq_count;
...
int ncm_vendor_add_response(void *app_ncm, u8 *buf, u32 len)
{
    struct f_ncm *ncm = (struct f_ncm *)app_ncm;
    int ret;
...
static inline void ncm_reset_values(struct f_ncm *ncm, int ntb16)
{
    if (ntb16) {
...
int ncm_vendor_notify(void *app_ncm, u32 cmd, void *param)
{
    struct f_ncm *ncm = (struct f_ncm *)app_ncm;
    int ret = 0;
    unsigned long flags;
...
/*USB ncm single interface for balong*/
static int ncm_set_alt_single_interface(struct usb_function *f, unsigned intf, unsigned alt)
{
    struct f_ncm        *ncm = func_to_ncm(f);
    struct usb_composite_dev *cdev = f->config->cdev;
    struct net_device   *net;
    ...
    if (alt == 0){
        ...
        if(bsp_usb_is_ncm_bypass_mode()){
            if(app_ctx&& app_ctx->bDataConnect && app_ctx->cbs.ndis_stat_change_cb){
        ...
        if (ncm->notify->driver_data) {
            ...
            if(!ncm->is_notify_complete){
                ncm->disep_at_req_incomp++;
            }
            ret = usb_ep_disable(ncm->notify);
            if(ret)
                GNET_ERR(("usb_ep_disable notify fail \n"));
        }

        if (!(ncm->notify->desc)) {
            USB_DBG_GADGET(&cdev->gadget->dev, "init ncm ctrl %d\n", intf);
            if (config_ep_by_speed(cdev->gadget, f, ncm->notify)){
                GNET_ERR(("config_ep_by_speed notify fail \n"));
                goto fail;
            }
        }
        
        ret = usb_ep_enable(ncm->notify);
        if(ret)
            GNET_ERR(("usb_ep_enable notify fail \n"));
				
        ncm->notify->driver_data = ncm;
        ncm->is_inited_speed = true;
		
        /*Directly call data interface process will cause a serial of problem including:
        1.A complex host-device time secquence problem that cause NCM notify 
        	endpoint fail in usb 3.0;
        2.PC driver bug cause it does not suspend usb when usb ncm netcard disconnect.
        So we simply disable spe port for usb and ignore other process like 
        gadget disconnt for a older version's process is like this and it works fine.
        */

#if (defined(CONFIG_BALONG_SPE) || defined(CONFIG_BALONG_SPE_MODULE))
        gether_spe_port_disable(&ncm->port);
#endif
        ...
        if (!ncm->port.in_ep->desc || !ncm->port.out_ep->desc) {
            ...
            if (config_ep_by_speed(cdev->gadget, f,ncm->port.in_ep) ||
                ...
                GNET_ERR(("config_ep_by_speed data fail\n"));
                goto fail;
            }
        }
...
static int ncm_set_alt(struct usb_function *f, unsigned intf, unsigned alt)
{
    ...
    if(ncm->is_single_interface){
        return ncm_set_alt_single_interface(f, intf, alt);
    }
    ...
    if (intf == ncm->ctrl_id){
        ...
        if (ncm->notify->driver_data) {
            USB_DBG_GADGET(&cdev->gadget->dev, "reset ncm control %d\n", intf);
            (void)usb_ep_disable(ncm->notify);
        }
...
static int ncm_get_ntb_format(struct gether *port)
{
    struct f_ncm    *ncm = func_to_ncm(&port->func);
    int ret = GNET_NTB32_FORMAT;
...
static void ncm_disable(struct usb_function *f)
{
    ...
    if(ncm->is_single_interface){
        ncm->is_inited_speed=false;
    }

    bsp_usb_set_enum_stat(ncm->data_id, 0);
    GNET_TRACE(("ncm deactivated\n"));
}
...
ncm_bind(struct usb_configuration *c, struct usb_function *f)
{
    ...
    int         status;
    struct usb_ep       *ep;
    struct usb_descriptor_header **fs_cur;
    struct usb_descriptor_header **hs_cur;
    struct usb_descriptor_header **ss_cur = NULL;
    ...
    else
    {
        if (ncm->is_class_balong)
        {
            ...
            ncm_control_intf.bInterfaceSubClass = bsp_usb_pc_driver_subclass_get(),
            ncm_control_intf.bInterfaceProtocol = USB_IF_PROTOCOL_NCM,
            ncm_data_intf.bInterfaceClass = USB_CLASS_VENDOR_SPEC,
            ncm_data_intf.bInterfaceSubClass = bsp_usb_pc_driver_subclass_get(),
    ...
    ss_ncm_notify_desc.bEndpointAddress = fs_ncm_notify_desc.bEndpointAddress;

    fs_cur = ncm->is_mbim_mode?ncm_mbim_fs_function:
            (ncm->is_single_interface?ncm_fs_function_single:ncm_fs_function);
    hs_cur = ncm->is_mbim_mode?ncm_mbim_hs_function:
            (ncm->is_single_interface?ncm_hs_function_single:ncm_hs_function);
    if (gadget_is_superspeed(c->cdev->gadget)) {
        ss_cur = ncm->is_mbim_mode?ncm_mbim_ss_function:
            (ncm->is_single_interface?ncm_ss_function_single:ncm_ss_function);
    }

    status = usb_assign_descriptors(f, fs_cur, hs_cur,
            ss_cur);
...
ncm_unbind(struct usb_configuration *c, struct usb_function *f)
{
    ...
    if(!the_ncm[ncm->port.port_no]){
        GNET_ERR(("ncm_unbind:null the_ncm!\n"));
        return ;
    }
    bsp_usb_del_setup_dev((unsigned)ncm->data_id);
        ...
        struct eth_dev *dev, int ntb16)
{
    ...
    if (ncm_string_defs[0].id == 0) {
        ...
        ncm_ecm_desc.iMACAddress = ncm_string_defs[STRING_MAC_IDX].id;
...
void f_ncm_dump(void)
{
    ...
    for (idx = 0; idx < GNET_MAX_NUM; idx++) {
        ...
        if (ncm) {
            ...
            printk("is_inited_speed:    %d\n", ncm->is_inited_speed);
            printk("is_notify_complete: %d\n", ncm->is_notify_complete);
            printk("notify_noreq_count: %d\n", ncm->notify_noreq_count);
            printk("host_timeout:       %d\n", ncm->host_timeout);
            printk("stat_host_timeout:  %d\n", ncm->stat_host_timeout);
            printk("disep_at_req_incomp:%d\n", ncm->disep_at_req_incomp);
