			  ...
			  struct ethtool_regs *regs, void *space)
{
	...
	if (!priv->plat->has_gmac) {
		...
		reg_space[22] = readl(priv->ioaddr + DMA_CHN_CUR_APP_TB(0));
		reg_space[23] = readl(priv->ioaddr + DMA_CHN_CUR_APP_RB(0));
	} else {
		/* MAC registers */
		for (i = 0; i < 55; i++)
			reg_space[i] = readl(priv->ioaddr + (i * 4));
			       ...
			       struct ethtool_coalesce *ec)
{
	...
	/* Appy to channel 0 */
	ec->tx_coalesce_usecs = tx_queue->tx_coal_timer;
	ec->tx_max_coalesced_frames = tx_queue->tx_coal_frames;

	if (rx_queue->use_riwt)
		ec->rx_coalesce_usecs = stmmac_riwt2usec(rx_queue->rx_riwt, priv);
			       ...
			       struct ethtool_coalesce *ec)
{
	...
	if ((rx_riwt > MAX_DMA_RIWT) || (rx_riwt < MIN_DMA_RIWT))
		return -EINVAL;
	else if (!rx_queue->use_riwt)
		return -EOPNOTSUPP;

	/* Only copy relevant parameters, ignore all others. */
	tx_queue->tx_coal_frames = ec->tx_max_coalesced_frames;
	tx_queue->tx_coal_timer = ec->tx_coalesce_usecs;
	rx_queue->rx_riwt = rx_riwt;
	priv->hw->dma->rx_watchdog(priv->ioaddr, rx_queue->rx_riwt, 0);
