...
ret_t rtl8367c_setAsicPortExtMode(rtk_uint32 id, rtk_uint32 mode)
{
    ...
    if( (mode == EXT_SGMII) || (mode == EXT_HSGMII) )
    {
        if( (retVal = rtl8367c_setAsicReg(RTL8367C_REG_SDS_INDACS_DATA, 0x7146)) != RT_ERR_OK)
            return retVal;
...
ret_t rtl8367c_setAsicPortRTCT(rtk_uint32 portmask)
{
    ...
    if(portmask > (0x0001 << RTL8367C_PHYNO))
		return RT_ERR_PORT_MASK;
    ...
    if((retVal = rtl8367c_setAsicRegBits(RTL8367C_REG_RTCT_ENABLE, RTL8367C_RTCT_ENABLE_PORT_MASK_MASK, portmask)) != RT_ERR_OK)
        return retVal;

    if((retVal = rtl8367c_setAsicRegBit(RTL8367C_REG_SEL_RTCT_PARA, RTL8367C_EN_RTCT_TIMOUT_OFFSET, 1)) != RT_ERR_OK)
        return retVal;

    if((retVal = rtl8367c_setAsicRegBit(RTL8367C_REG_SEL_RTCT_PARA, RTL8367C_EN_ALL_RTCT_OFFSET, 0)) != RT_ERR_OK)
        return retVal;

    if((retVal = rtl8367c_setAsicRegBit(RTL8367C_REG_SEL_RTCT_PARA, RTL8367C_DO_RTCT_COMMAND_OFFSET, 0)) != RT_ERR_OK)
        return retVal;

    if((retVal = rtl8367c_setAsicRegBit(RTL8367C_REG_SEL_RTCT_PARA, RTL8367C_DO_RTCT_COMMAND_OFFSET, 1)) != RT_ERR_OK)
        return retVal;
