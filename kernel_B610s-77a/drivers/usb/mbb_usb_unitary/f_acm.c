...
struct acm_name_type_tbl g_acm_cdev_type_table[ACM_CDEV_COUNT] = {
    ...
    {"acm_m2m_gps",     USB_IF_PROTOCOL_COMM_B},
...
struct acm_string_idx_protocol g_acm_string_table[] = {
    /*type(prot id)            port_string_idx */
    {USB_IF_PROTOCOL_PCUI, ACM_PCUI_STRING_IDX},
    {USB_IF_PROTOCOL_3G_PCUI, ACM_PCUI_STRING_IDX},
    {USB_IF_PROTOCOL_MODEM, ACM_MODEM_STRING_IDX},
    {USB_IF_PROTOCOL_3G_MODEM, ACM_MODEM_STRING_IDX},
    {USB_IF_PROTOCOL_DIAG, ACM_DIAG_STRING_IDX},
    {USB_IF_PROTOCOL_3G_DIAG, ACM_DIAG_STRING_IDX},
    {USB_IF_PROTOCOL_GPS, ACM_GPS_STRING_IDX},
    {USB_IF_PROTOCOL_3G_GPS, ACM_GPS_STRING_IDX},
    {USB_IF_PROTOCOL_COMM_B, ACM_SERIAL_B_STRING_IDX},
    {USB_IF_PROTOCOL_CTRL, ACM_CTRL_STRING_IDX},
};
...
static struct usb_string acm_string_defs[] = {
    ...
    [ACM_MODEM_IDX ].s = "Huawei Mobile Connect - Modem",
    [ACM_DIAG_IDX ].s = "Huawei Mobile Connect - Application",
    [ACM_PCUI_IDX ].s = "Huawei Mobile Connect - Pcui",
    [ACM_GPS_IDX ].s = "Huawei Mobile Connect - Gps",
    [ACM_GPS_CTRL_IDX].s = "Huawei Mobile Connect - Ctrl",
    [ACM_SERIAL_B_IDX].s = "Huawei Mobile Connect - Serial B",
	{  /* ZEROES END LIST */ },
};
...
static USB_INT acm_setup(struct usb_function *f, const struct usb_ctrlrequest *ctrl)
{
    ...
    case ((USB_DIR_OUT | USB_TYPE_VENDOR | USB_RECIP_INTERFACE) << 8) 
                       | USB_CDC_HUAWEI_REQ_SET_PORT_OPEN:
        value = w_length;
        cdev->gadget->ep0->driver_data = acm;
        req->complete = gs_acm_cdev_set_port_open_close;
        break;
	default:
invalid:
	...
	if (value >= 0) {
            DBG_I(MBB_ACM,"acm ttyGS%d req%02x.%02x v%04x i%04x l%d\n",
...
/*-------------------------------------------------------------------------*/
static USB_INT acm_interface_show_string_descriptor(struct usb_composite_dev* cdev, USB_INT protocol)
{
    USB_INT    status;
    huawei_port_string* port_string = NULL;
    USB_INT index = 0;
    USB_INT string_num = 0;
    ...
    for(index = 0;index < sizeof(g_acm_string_table) / sizeof(struct acm_string_idx_protocol); index++)
    {
        if(protocol == g_acm_string_table[index].type)
        {
            string_num = g_acm_string_table[index].port_string_idx;
            break;
        }
    }
    if(acm_string_defs[string_num + 2].id)
    {
        acm_single_interface_desc.iInterface =  acm_string_defs[string_num + 2].id;
    }
    else
    {
        ...
        acm_string_defs[string_num + 2].id = status;
        if (NULL != port_string)
        {
            acm_string_defs[string_num + 2].s = port_string->port_name[string_num];
        }
        else
        {
...
acm_bind(struct usb_configuration *c, struct usb_function *f)
{
    ...
    if (g_acm_is_single_interface) {
        ...
        if (!acm_interface_show_string_descriptor(cdev,acm->protocal))
        {
            DBG_E(MBB_ACM,"interface string descriptor report fail\n");
        }
    }
    else {
        acm->ctrl_id = status;
	...
	/* copy descriptors */
	f->fs_descriptors = usb_copy_descriptors(acm_fs_cur_function);
	if (!f->fs_descriptors)
		goto fail;
...
static inline USB_VOID acm_set_config_vendor(struct f_acm* acm)
{
    ...
    if (g_acm_is_single_interface)
    {
        ...
        if(pnp_api_handler->pnp_port_style_stat_cb 
            && pnp_api_handler->pnp_port_style_stat_cb())
        {
...
static acm_ctx_t  acm_ctx_var = 
{
    ...
    //.subclass_huawei_var = &g_subclass_huawei
};
...
static USB_INT acm_function_init(struct android_usb_function* f, struct usb_composite_dev* cdev)
{
    ...
    {
        DBG_I(MBB_ACM,"acm_function_init has been init\n");
        return 0;
    }
    else
    {
...
acm_function_bind_config(struct android_usb_function* f, struct usb_configuration* c)
{
    ...
    else if (!strcmp(f->name, "m2m_gps") && n_acm_cdev_ports > 0)
    {
        port_num = ACM_GET_PORT_NUM_FROM_PROTOCAL(USB_IF_PROTOCOL_COMM_B, acm_class_cdev);
        if(port_num < 0)
        {
            ret = port_num; 
        }
...
static struct android_usb_function acm_m2m_gps_function =
{
    .name           = "m2m_gps",
    .init           = acm_function_init,
    .cleanup        = acm_function_cleanup,
...
USB_VOID usb_acm_init (USB_VOID)
{
    ...
    pnp_register_usb_support_function(&acm_m2m_gps_function);
}
