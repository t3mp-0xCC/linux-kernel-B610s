...
/*и░идидж╠?ио?и▓Dии?иоии?mbb_usb_adp.h ;by w */
...
{
    struct gs_acm_modem_port *port = NULL;
    unsigned long flags;
    int status;
    ACM_MODEM_REL_IND_CB_T rel_ind_cb = NULL;
    ...
    if (NULL == port)
    {
        DBG_W(MBB_ACM, " modem port not exist, line:%d\n", __LINE__);
        return status;
    }

    rel_ind_cb = port->rel_ind_cb;
    if (rel_ind_cb)
    {
        rel_ind_cb(false);
    }
...
long gs_acm_modem_ioctl(struct file *file, unsigned int cmd, unsigned long arg)
{
    ...
    case ACM_MODEM_IOCTL_SET_REL_IND_CB:
	    ...
	    break;
#if ( FEATURE_ON == MBB_HSUART_AT )
    case ACM_MODEM_IOCTL_SET_INBOUND_FLOW_CONTROL:
    {
        pr_err("USB MDM SET_INBOUND_FLOW_CONTROL %d\n",arg);
        port->flow_ctrl = (unsigned int)arg;
        if ( 0 ==  port->flow_ctrl )
        {
            USB_ULONG flags;/*lint !e578*/
            printk(KERN_ERR"USB start to read\n");
            /*lint -e160 -e530 -e10 -e550 -e522 -e666*/
            spin_lock_irqsave(&port->port_lock, flags);
            /*lint +e160 +e530 +e10 +e550 +e522 +e666*/
            gs_acm_modem_start_rx(port);
            spin_unlock_irqrestore(&port->port_lock, flags);
...
{
    int error;
    dev_t dev = 0; //fix coverity 20150420
...
static acm_ctx_t* acm_ctx_mdm =NULL;
...
USB_VOID gacm_modem_cleanup(USB_VOID)
{
	...
	for (i = 0; i < gs_acm_modem_n_ports; i++) {
		...
		//wait_event(port->close_wait, gs_acm_modem_closed(port));
         wait_event_interruptible(port->close_wait, gs_acm_modem_closed(port));
...
void acm_modem_dump(int port_num)
{
    ...
    DBG_E(MBB_ACM,"dev name                  %s\n", 
            acm_ctx_mdm->mdm_name_type_var[port_num].name);
