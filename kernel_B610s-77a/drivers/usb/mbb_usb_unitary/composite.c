...
static USB_UINT g_curr_cfg_idx = 0;

void inline usb_set_curr_cfg_idx(u32 cfg_idx)
{
    DBG_T(MBB_DEBUG, "g_curr_cfg_idx= %d,cfg_idx=%d\n", g_curr_cfg_idx, cfg_idx);
    g_curr_cfg_idx = cfg_idx;
}


u32 inline usb_get_curr_cfg_idx(void)
{
    if (0 == g_curr_cfg_idx)
    {
        DBG_T(MBB_DEBUG, "g_curr_cfg_idx= %d\n", g_curr_cfg_idx);
    }
    return g_curr_cfg_idx;
}
                         ...
                         struct usb_function* function)
{
    ...
    DBG_I(MBB_PNP, "adding '%s'/%p to config '%s'/%p\n",
                         ...
                         struct usb_function* function)
{
    ...
    if (id < MAX_CONFIG_INTERFACES)
    {
        ...
        DBG_I(MBB_PNP, "config %x, id %d, next_interface_id %d\n", config, id, config->next_interface_id);
        return id;
    }
    return -ENODEV;
}
...
static USB_INT bos_desc(struct usb_composite_dev* cdev)
{
    ...
    usb_ext->bmAttributes = 0;//cpu_to_le32(USB_LPM_SUPPORT);
...
static USB_INT get_config_functions_number(struct usb_configuration* c)
{
    struct usb_function* f_tmp = NULL;
    USB_INT function_count = 0;

    list_for_each_entry(f_tmp, &c->functions, list)
    {
        function_count++;
    }
                          ...
                          const struct usb_ctrlrequest* ctrl, USB_UINT number)
{
    ...
    if ((usb_get_mbim_config() == number) && (UsbCtx->usb_switch_gatway_mode_cb))
    {
    ...
    usb_set_curr_cfg_idx(number);
    set_value_usb_setup_num(get_config_functions_number(c));
    ...
    for (tmp = 0; tmp < MAX_CONFIG_INTERFACES; tmp++)
    {
        ...
        {
            DBG_I(MBB_PNP, "interface %d (%s) requested delayed status\n",
                  tmp, f->name);
            cdev->delayed_status++;
...
#ifdef USB_CHARGE
    /*usb_set_speed(gadget->speed);*/
    charger_type = MBB_USB_CHARGER_TYPE_PC;
    usb_charger_type_detect_event(NULL, MBB_USB_DEVICE_IDEN, &charger_type);
#endif /* USB_CHARGE */
    huawei_set_usb_enum_state(USB_ENUM_DONE);
    /* 此处给应用和充电模块上报USB插入事件*/
    usb_notify_syswatch(EVENT_DEVICE_USB, USB_ATTACH_EVENT);
    /* when we return, be sure our power usage is valid */
    power = c->MaxPower ? (2 * c->MaxPower) : CONFIG_USB_GADGET_VBUS_DRAW;
done:
                       ...
                       USB_INT (*bind)(struct usb_configuration*))
{
    ...
    DBG_I(MBB_PNP, "adding config #%u '%s'/%p\n",
          config->bConfigurationValue,
          config->label, config);

    //set curr ConfigurationValue
    usb_set_curr_cfg_idx(config->bConfigurationValue);
    ...
    else
    {
        ...
        DBG_I(MBB_PNP, "cfg %d/%p support speeds:%s%s%s\n",
        ...
        list_for_each_entry(androidfun, &config->android_functions, enabled_list)
        {
            if (i < MAX_CONFIG_INTERFACES)
            {
                ...
                DBG_I(MBB_PNP, "interface%d name = %-10s type:%-25s /%p\n",
                          ...
                          struct usb_configuration* config)
{
    while (!list_empty(&config->functions))
    {
        ...
        if (f->unbind)
        {
            DBG_I(MBB_PNP, "unbind function '%s'/%p\n", f->name, f);
            f->unbind(config, f);
            /* may free memory for "f" */
        }
    }
    if (config->unbind)
    {
        DBG_I(MBB_PNP, "unbind config '%s'/%p\n", config->label, config);
                       ...
                       struct usb_configuration* config)
{
    ...
    USB_INT faston_off = MBB_USB_NO;
#endif/*USB_FAST_ON_OFF*/
...
#endif /* #ifdef  MBB_USB_RESERVED  mhl del*/
    ...
    switch (ctrl->bRequest)
    {
        ...
        case USB_REQ_SET_CONFIGURATION:
            ...
            if (gadget_is_otg(gadget))
            {
                ...
                { DBG_I(MBB_PNP, "HNP available\n"); }
                else if (gadget->a_alt_hnp_support)
                { DBG_I(MBB_PNP, "HNP on another port\n"); }
                else
                { DBG_I(MBB_PNP, "HNP inactive\n"); }
            }
            /*### mlog输出PID*/
        ...
        case USB_REQ_SET_INTERFACE:
            ...
            {
                DBG_E(MBB_PNP, "interface %d (%s) requested delayed status\n",
                      intf, f->name);
                cdev->delayed_status++;
...
void usb_composite_unregister(struct usb_composite_driver* driver)
{
    if (composite != driver)
    {
        DBG_E(MBB_PNP, "composite(=%p) != driver(=%p)\n", composite, driver);
        return;
    }
...
void usb_composite_setup_continue(struct usb_composite_dev* cdev)
{
    ...
    {
        DBG_E(MBB_PNP, "Unexpected call\n");
    }
    else if (--cdev->delayed_status == 0)
    {
        DBG_I(MBB_PNP, "Completing delayed status\n");
        req->length = 0;
