...
#define USB_DEVICE_EMBEDDED_IDX             2

enum ncm_device_type {
    NCM_DEVICE_TYPE_UNKNOWN = 0,
    NCM_DEVICE_TYPE_EMBEDDED,       /* modem */
    NCM_DEVICE_TYPE_REMOVABLE,        /* datacard */
    NCM_DEVICE_TYPE_MAX,
};
...
USB_VOID ncm_cut_response_size(struct f_ncm *ncm, USB_UINT8 *buf, USB_UINT32 size)
{
    ...
    list_for_each_entry_safe(resp, next, &ncm->params.resp_queue, list)
    {
        if (resp->buf == buf) 
        {
            ...
            {
                DBG_E(MBB_NCM, "can not cut buf %p size form %u to %lu\n",
...
USB_BOOL ncm_get_bypass_state(USB_VOID)
{
    ...
    bRet = (GATEWAY_OS_FLAG == pc_type);  /* E5 在网关下上报bypass模式网口 */
#else
    bRet = true;
#endif
...
USB_VOID ncm_set_mbim_mode(USB_INT switch_idex)
{
    ...
    usb_nv_info->debug_mode.setmode_flag = switch_idex;
    ret = usb_write_nv(USB_NV_USB_DEBUG_MODE_FLAGE, &usb_nv_info->debug_mode, 
                           sizeof(nv_huawei_debug_mode_flag));
    if(MBB_USB_OK != ret)
    {
        DBG_E(MBB_PNP, "mbim debug mode flag NV write fail!\n");
    }
#else
...
USB_UINT ncm_get_mbim_mode(USB_VOID)
{
    ...
    return (USB_UINT32)nv_st->debug_mode.setmode_flag;
#else
...
USB_UINT ncm_get_mbim_mode(USB_VOID)
{
    DBG_T(MBB_NCM, "not with mbim ,stub!");
    return 0;
}
...
static inline USB_VOID ncm_reset_values(struct f_ncm *ncm, USB_INT ntb16)
{
    if (ntb16) {
...
static USB_INT ncm_get_device_type(USB_VOID)
{
#ifdef MBB_USB_SOLUTION
    return NCM_DEVICE_TYPE_EMBEDDED;
#else
    return NCM_DEVICE_TYPE_REMOVABLE;
#endif
}    
...
static USB_INT ncm_setup(struct usb_function *f, const struct usb_ctrlrequest *ctrl)
{
    ...
    if (!VERIFY_NCM_BYPASS(ncm))
    {
        if ((USB_CDC_SEND_ENCAPSULATED_COMMAND == ctrl->bRequest)
          ...
          || (USB_CDC_GET_HUAWEI_DEVICE_TYPE == ctrl->bRequest))
        {
            goto invalid;
        }
    }  
    ...
    */
        ...
        | USB_CDC_SET_NTB_FORMAT):
    {
        ...
        /* some host driver don't set correct, we can use force ntb16 here */
        if (GNET_NTB16_FORCE_FORMAT == ncm->ntb16_fmt) 
        {
            ncm->parser_opts = &ndp16_opts;
        }
        ncm->ntb16_fmt = (&ndp16_opts == ncm->parser_opts);
    ...
    case (((USB_DIR_IN | USB_TYPE_VENDOR | USB_RECIP_INTERFACE) << 8)
      | USB_CDC_GET_HUAWEI_DEVICE_TYPE):
    {
        if (w_length != 1 || w_value != 0 || w_index != ncm->ctrl_id)
        {
            goto invalid;
        }

        if (NCM_DEVICE_TYPE_EMBEDDED == ncm_get_device_type())
        {
            *(UINT8*)(req->buf) = USB_DEVICE_EMBEDDED_IDX;
        }
        else
        {
            goto invalid;
...
static USB_INT ncm_set_alt(struct usb_function *f, USB_UINT intf, USB_UINT alt)
{
    ...
    if (((intf == ncm->ctrl_id) && (0 == alt) && (true == ncm->is_single_interface))
        || ((intf == ncm->ctrl_id) && (false == ncm->is_single_interface)))
    {

        if ((0 != alt) && (false == ncm->is_single_interface))
        {
            goto fail;
        }
        ...
        if ((0 == alt) && ( 0 == (VERIFY_NCM_BYPASS(ncm))))
        {
    ...
    else if (intf == ncm->data_id)
    {
        ...
        if (ncm->port.in_ep->driver_data)
        {
            ...
            ncm_reset_values(ncm, ncm->ntb16_fmt);
        }
    ...
    if (VERIFY_NCM_MBIM(ncm))
    {
        ndp16_opts.ndp_sign = USB_CDC_MBIM_NDP16_IPS_SIGN;
        ndp32_opts.ndp_sign = USB_CDC_MBIM_NDP32_IPS_SIGN;
    }
    else
    {
    ...
    {
        if(pnp_api_handler->pnp_get_ctl_app_flag_cb)
        {
            if (pnp_api_handler->pnp_set_ctl_app_flag_cb)
            {
                pnp_api_handler->pnp_set_ctl_app_flag_cb(CTL_APP_DISABLE);
            }
        }
    }
...
{
#ifdef CONFIG_GNET_PREALLOC_TX_MEM
    if((ctx->ntb) && (ctx->ntb->addr) && (ctx->ntb->actual))
    {
        memset(ctx->ntb->addr, 0, ctx->ntb->actual);
        ctx->ntb->actual = 0;
    }
#else
    if(ctx->ntb)
        ...
        if(dev->ctx.quirks & GNET_NCM_QUIRK_DWC_TRB_CACHE)
        {
            ...
            else
            {
                ...
                if((USB_CDC_NCM_DWC_CACHE_TRB_NUM - 1) == trbs_for_quirk)
                {
                    ...
                    #ifdef  MBB_USB_RESERVED   /* when quirk occur,align is not necessary */
                    ...
                    struct sk_buff *skb)
{
    ...
    tx_ctx_t *ctx;
    USB_ULONG flags;
    USB_UINT32 pkt_len_waterline = dev->ctx.pkt_len_waterline;
    if (GNET_NCM_ROOM_FOR_NTB_INMAXSIZE <= port->fixed_in_len)
    {
        pkt_len_waterline = min(port->fixed_in_len - GNET_NCM_ROOM_FOR_NTB_INMAXSIZE, dev->ctx.pkt_len_waterline);
    }
    else
    {
        /*fixed_in_len is [2048, 128K], it is larger than (1514+44)*/
        pkt_len_waterline = min(port->fixed_in_len - (MAX_SEG_SIZE + IP_HEADER_SIZE), dev->ctx.pkt_len_waterline);
    }
...
static USB_INT ncm_get_ntb_format(struct gether *port)
{
    struct f_ncm *ncm = func_to_ncm(&port->func);
    USB_INT ret = GNET_NTB32_FORMAT;

    if (ncm) 
    {
        ret = (ncm->parser_opts == &ndp16_opts) ?
            GNET_NTB16_FORMAT : GNET_NTB32_FORMAT;
    }
    return ret;
}
...
ncm_bind(struct usb_configuration *c, struct usb_function *f)
{
    ...
    usb_pnp_api_hanlder *pnp_api_handler = pnp_adp_get_api_handler();
    USB_UINT8 pc_type = OS_FLAG_INVALID;
    USB_UINT8 gateway_mode = GATEWAY_MODE_INVALID;
    ...
    if(pnp_api_handler->pnp_get_gateway_mode_cb)
    {
        gateway_mode = pnp_api_handler->pnp_get_gateway_mode_cb();
    }
    DBG_T(MBB_NCM, "config %u\n", c->bConfigurationValue);
    
    if (usb_get_mbim_config() == c->bConfigurationValue)    /* config 2 or 3 for MBIM */
    {
    ...
    else if (GATEWAY_NDIS_MODE == gateway_mode)
    {
    ...
    f->fs_descriptors = usb_copy_descriptors(desc);
    if (!f->fs_descriptors)
        goto fail;
...
USB_INT ncm_bind_config(struct usb_configuration *c, 
  struct android_usb_function* f, USB_UINT8 ethaddr[ETH_ALEN],  struct eth_dev *dev)
{
    struct f_ncm    *ncm;
    USB_INT       status;
    USB_INT       index = 0;
    huawei_port_string* port_string = NULL;
#ifdef CONFIG_NCM_MBIM_SUPPORT
    mbb_usb_nv_info_st *nv_st = usb_nv_get_ctx();
#endif
    ...
    if (index >= 0)
    {
        port_string = usb_device_desc_string_get(index);
    }
    ...
    if (ncm_string_defs[0].id == 0) {
        ...
        if (NULL != port_string)
        {
            ncm_string_defs[STRING_CTRL_IDX].s = port_string->port_name[NCM_STRING_IDX];
        }
        else
        {
            DBG_E(MBB_ACM,"port_string null\n");
        }
    ...
    ncm->port.get_wrap_format = ncm_get_ntb_format;
    ...
    ncm->port.ioport = (struct eth_dev *)dev;
...
USB_VOID f_ncm_dump(USB_VOID)
{
    ...
    for (idx = 0; idx < GNET_MAX_NUM; idx++) {
        ...
        if (ncm) {
            ...
            DBG_T(MBB_NCM, "ncm format:         %s\n",
                (ncm->parser_opts == &ndp16_opts) ? "NTB16":"NTB32");
        }
    }
}
#if (defined(CONFIG_BALONG_SPE) || defined(CONFIG_BALONG_SPE_MODULE))
...
{
	u16 div = le16_to_cpu(ntb_parameters.wNdpInDivisor);
	u16 rem = le16_to_cpu(ntb_parameters.wNdpInPayloadRemainder);
    u32 align_parameter;
	align_parameter = div;
	align_parameter = align_parameter << 16;
	align_parameter += rem;
	return align_parameter;
}
USB_UINT32 ncm_get_max_length(USB_VOID)
{
    u32 max_size = le32_to_cpu(ntb_parameters.dwNtbOutMaxSize);
	return max_size;
}
#endif
    ...
    struct android_usb_function* f, struct usb_configuration* c)
{
    ...
    for (loop = 0; loop < port_num; loop++)
    {
        ...
        dev = gether_setup(c->cdev->gadget, hostaddr[loop]);
        if (IS_ERR(dev)) {
            ret = PTR_ERR(dev);
        ...
        ret = ncm_bind_config(c, f, hostaddr[loop], dev);
        if (ret < 0)
        {
...
/*
    ...
    {
        DBG_I(MBB_NCM, "%s: bind %d\n", __func__, loop);
        ret = ncm_bind_config(c, f, hostaddr[loop], get_last_ether_dev());
        if (ret < 0)
        {
            DBG_E(MBB_NCM, "ncm_bind_config %d fail!\n", loop);
            goto clean_gnet_config;
...
#if (defined(CONFIG_BALONG_SPE) || defined(CONFIG_BALONG_SPE_MODULE))
    if (0 > eth_adp_for_spe_init(NULL, cdc_type))
    {
        DBG_E(MBB_NCM, "ncm spe adp init failed\n");
    }
    DBG_T(MBB_NCM, "ncm spe port alloc suscess\n");
#endif
}
