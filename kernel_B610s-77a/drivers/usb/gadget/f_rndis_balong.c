...
rndis_unbind(struct usb_configuration *c, struct usb_function *f)
{
    ...
    bsp_usb_del_setup_dev((unsigned)rndis->data_id);
		...
		u32 vendorID, const char *manufacturer, struct eth_dev *dev)
{
	...
	rndis->port.fixed_out_len = GNET_OUT_MAXSIZE_PER_TRANSFER;
	rndis->port.fixed_in_len = GNET_IN_MAXSIZE_PER_TRANSFER;
	rndis->port.wrap = rndis_wrap_single;
#ifdef CONFIG_GNET_PREALLOC_RX_MEM
	rndis->port.unwrap = rndis_unwrap_array;
#else	/*!CONFIG_GNET_PREALLOC_RX_MEM*/
