...
extern void syssc_usb_vbusvldext(int value);
extern void syssc_usb_override(int value);
...
extern void sysctrl_set_usbid(int usbid);
static int dwc_otg2_probe(struct platform_device *pdev)
{
	struct dwc_otg2 *otg;
    struct usb_phy *phy;
	int err;
    ...
    phy = devm_usb_get_phy(&pdev->dev, USB_PHY_TYPE_USB3);
    if(IS_ERR(phy)){
        dev_err(&pdev->dev, "failed to devm_usb_get_phy\n");
        return -ENOENT;
    }
...
#ifdef CONFIG_USB_OTG_USBID_BYGPIO
	sysctrl_set_usbid(otg_get_usbid());
#endif
...
static int dwc_otg2_remove(struct platform_device *pdev)
{
...
static const struct of_device_id dwc3_otg3_match[] = {
...
static struct platform_driver dwc_otg2_xceiv_driver = {
	...
	.driver	= {
		.name	= "dwc3_otg3",
        .of_match_table	= of_match_ptr(dwc3_otg3_match),
		.owner	= THIS_MODULE,
	},
};
