...
static int pl011_fifo_to_tty(struct uart_amba_port *uap)
{
	...
	while (max_count--) {
		...
		if (uart_handle_sysrq_char(&uap->port, ch & 255))
			continue;
#if (((FEATURE_OFF == MBB_BUILD_DEBUG) && (FEATURE_OFF == MBB_FACTORY) && (defined(BSP_CONFIG_BOARD_E5_DC04) || defined(BSP_CONFIG_BOARD_CPE_KD11))) \
    || ((FEATURE_ON == MBB_PHY_UART_TEST) && (FEATURE_ON == MBB_FACTORY)))
        /* DC04 非debug且非烧片版本中串口不输入；烧片版本中开启物理串口回环测试时，不处理串口输入 */
#else
	...
	if (status) {
		do {
			if (uap->vendor->cts_event_workaround) {
				...
				writel(0x00, uap->port.membase + UART011_ICR);
				 ...
				 */
				dummy_read = readl(uap->port.membase + UART011_ICR);
				dummy_read = readl(uap->port.membase + UART011_ICR);
			}
...
static int pl011_hwinit(struct uart_port *port)
{
	 ...
	 */
	uap->im = readl(uap->port.membase + UART011_IMSC);
	writel(UART011_RTIM | UART011_RXIM, uap->port.membase + UART011_IMSC);
...
static int pl011_startup(struct uart_port *port)
{
	...
	/* Clear out any spuriously appearing RX interrupts */
	 writel(UART011_RTIS | UART011_RXIS,
		uap->port.membase + UART011_ICR);
	uap->im = UART011_RTIM;
	if (!pl011_dma_rx_running(uap))
		uap->im |= UART011_RXIM;
...
static int pl011_probe(struct amba_device *dev, const struct amba_id *id)
{
	...
	writel(0, uap->port.membase + UART011_IMSC);
	writel(0xffff, uap->port.membase + UART011_ICR);
#endif
