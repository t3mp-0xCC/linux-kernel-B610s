...
*******************************************************************************/
#define FTS_META_REGS        3  //the mean?
...
#define focal_ldo_en_index (0)
#define focal_int_en_index (1)
#define focal_rst_en_index (2)
#define focal_num_max_touches (3)
#define focal_platform_data_length (4)

#define focal_max_xcoor (320)
#define focal_max_ycoor (240)
#define FTS_POWER_ON_DELAY (2)
#define FTS_SOFT_RESET      (10)
#define FTS_HARD_RESET_DELAY  (200)




static int focal_init_status = -1;
...
/*******************************************************************************
*  Name: focal_fw_version_read
*  Brief:  read focal touch firmeware version
*  Input: NA
*  Output: NA
*  Return:  NA
*******************************************************************************/
void focal_fw_version_read (void)
{
    u8 version = 0;
    int ret;
    ...
    ret = fts_read_reg(fts_i2c_client, FTS_REG_FW_VER, &version);
    if(ret < 0)
    {
        dev_err(&fts_i2c_client->dev, "%s: i2c read error.err_no is %d\n", __func__,ret);
    }
    if (1 == focal_debug_flag)
    {
        printk(KERN_ERR "*****Focal TP FW version=0x%x*****\n",version);
    }
}
/*******************************************************************************
*  Name: focal_debug_flag_set
*  Brief:
*  Input:
*  Output: 
*  Return: 
*******************************************************************************/
void focal_debug_flag_set(int flag)
{
  focal_debug_flag = flag;
}
/*******************************************************************************
...
#endif
/*******************************************************************************
*  Name: fts_gpio_request_config
*  Brief: set gpio to the request status
*  Input:
*  Output: 
*  Return: 
*******************************************************************************/
...
int fts_i2c_read(struct i2c_client *client, char *writebuf, int writelen, char *readbuf, int readlen)
{
    ...
    if (writelen > 0) {
        ...
        if (ret < 0)
            dev_err(&client->dev, "%s:i2c read error.err_no is %d\n", __func__,ret);
    }
...
static void fts_report_value(struct fts_ts_data *data)
{
    ...
    for (i = 0; i < FTS_MAX_POINTS; i++) {
        ...
        if(1 == focal_debug_flag)
        {
            printk(KERN_ERR "ABS_MT_POSITION_X is %d\n",event->au16_x[i]);
            printk(KERN_ERR "ABS_MT_POSITION_Y is %d\n",event->au16_y[i]);
...
/*******************************************************************************
*  Name: fts_init_gpio_configure
*  Brief:
*  Input: the pointer of gpio resource 
*  Output: 
*  Return: the result of function 0:success other:fail
*******************************************************************************/
static int fts_init_gpio_configure(struct fts_ts_platform_data *pdata)
{
    int err = 0;
    ...
    if (gpio_is_valid(pdata->irq_gpio)) {
        err = gpio_request(pdata->irq_gpio,
                    "fts_irq_gpio");
             if (err) {
                printk(KERN_ERR " %s irq gpio request failed",__func__);
                goto err_irq_gpio_request_handle;

             }
    }
    
    if (gpio_is_valid(pdata->ldo_en)) {
        err = gpio_request(pdata->ldo_en,
                    "fts_ldo_gpio");
             if (err) {
                printk(KERN_ERR "ldo-en gpio request failed");
                goto err_ldo_en_gpio_handle;
    ...
    if (gpio_is_valid(pdata->reset_gpio)) {
        err = gpio_request(pdata->reset_gpio,
                    "fts_reset_gpio");
             if (err) {
                printk(KERN_ERR "reset gpio request failed");
                goto err_reset_gpio_handle;
...
err_reset_gpio_handle:
    if (gpio_is_valid(pdata->ldo_en))
        gpio_free(pdata->ldo_en);
err_ldo_en_gpio_handle:
    if (gpio_is_valid(pdata->irq_gpio))
        gpio_free(pdata->irq_gpio);
err_irq_gpio_request_handle:
    return err;
...
static int fts_power_on(struct fts_ts_data *data, bool on)
{
    ...
    rc = gpio_direction_output((unsigned int)(data->pdata->ldo_en),1);
    if(rc != 0)
    {
        printk(KERN_ERR "ldo_en set to low fail\n");
        goto fail;
    }
    msleep(2);
    rc = gpio_direction_output((unsigned int)(data->pdata->reset_gpio),1);/*lint !e534*/
    if(rc != 0)
    {
        printk(KERN_ERR "reset_gpio set to low fail\n");
        goto fail;
}

    msleep(FTS_HARD_RESET_DELAY);
    return rc;
...
power_off:
   ...
   /*power off vcc*/
    rc = gpio_direction_output((unsigned int)(data->pdata->reset_gpio),0);/*lint !e534*/
    if(rc != 0)
    {
        printk(KERN_ERR "reset set to low fail\n");

    }
    rc = gpio_direction_output((unsigned int)(data->pdata->ldo_en),0);
    if(rc != 0)
    {
        printk(KERN_ERR "ldo_en set to low fail\n");
        goto fail;
    ...
    if(1 == focal_debug_flag)
    {
       printk("fts_power_off reset gpio and ldo_en is set to low\n");
    }
    return rc;
fail:
    gpio_direction_output((unsigned int)(data->pdata->reset_gpio),1);
    return rc;
}
...
static int fts_ts_start(struct device *dev)
{
    ...
    if(1 == focal_debug_flag)
    {
        printk("focal resume is complete \n");
    }
...
int fts_ts_suspend(struct device *dev)
{
    ...
    if(1 == focal_debug_flag)
    {
       printk(KERN_ERR "fts_ts_suspend is enter\n");
    }
...
{
    printk("enter stub fts_ts_suspend\n");
    return 0;
}
/*******************************************************************************
*  Name: fts_ts_resume
...
{
    printk("enter stub fts_ts_resume\n");
    return 0;
}
#endif
...
static int fts_ts_probe(struct i2c_client *client, const struct i2c_device_id *id)
{
    ...
    #ifdef CONFIG_TOUCHSCREEN_FTS_PSENSOR //这个宏的作用
    struct fts_psensor_platform_data *psensor_pdata;
    struct input_dev *psensor_input_dev;
    #endif
    u32 reg_addr_DT[focal_platform_data_length] = {0};
    ...
    if(NULL == client)
    {
        printk(KERN_ERR "%s: client or id is NULL!!!\n", __func__); 
        return -ENOMEM;            
    ...
    if (client->dev.of_node) {
        ...
        dev_err( &client->dev,"[%s:%d] device tree information value get fail\n", __func__,__LINE__);
        return - ENODEV;
    }
    
    err = of_property_read_u32_array(client->dev.of_node, "gpios", reg_addr_DT, focal_platform_data_length); 
    if(err)
    {
        dev_err( &client->dev,"[%s:%d] device tree information value get fail\n", __func__,__LINE__);
        return - ENODEV;
    ...
    /*获取上电参数*/
    pdata->ldo_en          = reg_addr_DT[focal_ldo_en_index];
    pdata->reset_gpio      = reg_addr_DT[focal_rst_en_index];
    pdata->irq_gpio        = reg_addr_DT[focal_int_en_index];
    pdata->num_max_touches = reg_addr_DT[focal_num_max_touches];
    ...
    input_set_abs_params(input_dev, ABS_MT_TRACKING_ID, 0, FTS_TOUCH_POINT_NUM - 1, 0, 0);
    input_set_abs_params(input_dev, ABS_MT_POSITION_X, 0, focal_max_xcoor, 0, 0);
    input_set_abs_params(input_dev, ABS_MT_POSITION_Y, 0, focal_max_ycoor, 0, 0);
    ...
    #endif
    /*上电init*/
    err = fts_init_gpio_configure(pdata);

    if(err)
    {
        printk(KERN_ERR "%s: Failed to get  gpios for focal init \n",
            __func__);
    ...
    gpio_direction_output((unsigned int)(pdata->ldo_en),1);/*lint !e534*/

    
    msleep(FTS_POWER_ON_DELAY);
    /* 申请并配置RESET使用的GPIO */

    gpio_direction_output((unsigned int)(pdata->reset_gpio),1);/*lint !e534*/
    ...
    if (err < 0) {
        dev_err(&client->dev, "Device ID read failed");
        goto free_gpio;
    }
    ...
    err = request_irq(client->irq, fts_ts_interrupt, IRQF_ONESHOT | IRQF_TRIGGER_FALLING, client->dev.driver->name, data);
...
free_gpio:
    ...
    if (gpio_is_valid(pdata->reset_gpio))
    {
        gpio_direction_output((unsigned int)(pdata->reset_gpio),0);
        msleep(1);
    ...
    if (gpio_is_valid(pdata->ldo_en))
    {
        gpio_direction_output((unsigned int)(pdata->ldo_en),0);
        msleep(1);     
        gpio_free(pdata->ldo_en);
    }
...
static int __init fts_ts_init(void)
{
    ...
    if (ret < 0)
    {
        printk(KERN_ERR "add touch device driver i2c driver failed.\n");
        return -ENODEV;
    }
...
int touch_fts_init(void)
{
    ...
    ret = focal_init_status;
    return ret;
}

void touch_fts_exit(void)
{
    fts_ts_exit();
}
