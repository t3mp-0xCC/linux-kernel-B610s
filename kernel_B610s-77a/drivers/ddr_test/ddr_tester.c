...
int put_log( const char *fmt, ...)
{
    ...
    va_start(args, fmt);
    r = vsnprintf( line, sizeof(line) - 1, fmt, args );
    ...
    {
        // buf 大小来源 (char *) __get_free_pages(gfp_flags, M_ORDER );
        // 其中，参数 M_ORDER 是要分配的页面数的以2为底的对数。
        // 故 buf 最大空间为 PAGE_SIZE * ( 1<<M_ORDER)，即 M_BLOCK_SIZE
        snprintf( &buf[pos_offset] , M_BLOCK_SIZE - pos_offset, "%s", line );
    }
