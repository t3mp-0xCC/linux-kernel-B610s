...
u32 g_tmc_data_addr = 0;
u32 g_tmc_data_len  = 0;
...
static void __tmc_etb_dump(struct tmc_drvdata *drvdata)
{
    ...
    g_tmc_data_addr = (u32)drvdata->buf;
	...
	while (1) {
		for (i = 0; i < memwords; i++) {
			...
			{
                g_tmc_data_len = (u32)(bufp - drvdata->buf);
				return;
			}
            ...
            {
                printk("_tmc_etb_dump error: read TMC_RDD longer than buffer size.\n");
                return;
            }
		}
	}
