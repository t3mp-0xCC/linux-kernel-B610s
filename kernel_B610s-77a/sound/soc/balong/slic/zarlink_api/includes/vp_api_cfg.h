 ...
 *
 * $Revision: 12022 $
 * $LastChangedDate: 2016-05-05 15:15:49 -0500 (Thu, 05 May 2016) $
 */
...
#define VP886_EVENT_QUEUE_SIZE 8
...
/* VP886_INCLUDE_MPI_QUICK_TEST
   ...
   during VpInitDevice().  This can be #undef to save on code size and MPI
   traffic at initialization after a design is proven to work. */
#undef VP886_INCLUDE_MPI_QUICK_TEST
...
/* Define to enable a mode where the sampling buffers are read via a separate
   function Vp886DtmfAsyncSampling() instead of during VpGetEvent(). The intent
   is to allow the sampling reads to be performed in a higher priority task in
   cases where calling VpGetEvent() at a regular rate might not be possible. */
#define VP886_DTMF_ASYNC_SAMPLING

/* Number of samples to store in the line object from the asynchronous sampling
   function. Storage requirement is (3 bytes * VP886_DTMF_ASYNC_BUFFER_SIZE) per
   line object.
   (Maximum value 65535) */
#define VP886_DTMF_ASYNC_BUFFER_SIZE 4000
...
/* Error-checking of DTMF_DETECT definitions */
    ...
    #if defined(VP886_DTMF_DETECT_NONCONTINUOUS) && defined(VP886_DTMF_ASYNC_SAMPLING)
        #error Must not define both VP886_DTMF_DETECT_NONCONTINUOUS and VP886_DTMF_ASYNC_SAMPLING
    #endif
#endif
