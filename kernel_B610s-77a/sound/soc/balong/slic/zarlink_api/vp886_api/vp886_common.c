 ...
 *
 * $Revision: 11528 $
 * $LastChangedDate: 2014-08-25 14:32:24 -0500 (Mon, 25 Aug 2014) $
 */
    ...
    *pOverflow = (buf[0] & VP886_R_B1_OVFL) >> 6;
    ...
    for (i = 0; i < *pLength; i++) {
        ...
        if (*pOverflow == 0) {
            ...
            /* The samples AFTER ptr were taken first and need to go at the
               beginning.  The samples up to ptr were taken after the overflow
               occurred, so they go after.  The formula (i + 60 - ptr) % 60
               rotates the buffer samples to the left by ptr to achieve this. */
           pBuffer[(i + 60 - ptr) % 60] = (int16)sample;
        }
    }
