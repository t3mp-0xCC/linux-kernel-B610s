...
{
    if(VP_CODE_CRITICAL_SEC == criticalSecType)
    {
        osl_sem_down(&slic_priv->code_sem);
    }
    else if(VP_MPI_CRITICAL_SEC == criticalSecType)
    {
        osl_sem_down(&slic_priv->mpi_sem);
    }
    ...
    VpCriticalSecType criticalSecType)
{
    if(VP_CODE_CRITICAL_SEC == criticalSecType)
    {
        osl_sem_up(&slic_priv->code_sem);
    }
    else if(VP_MPI_CRITICAL_SEC == criticalSecType)
    {
        osl_sem_up(&slic_priv->mpi_sem);
    }
    return 0;
} /* VpSysExitCritical() */
