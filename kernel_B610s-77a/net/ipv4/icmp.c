...
#include "./../netlink/kcmsmonitormsgtypes.h"
#define NIPQUAD(addr) \
...
#define DEVICE_ID_SCANATTACK 29
extern int log_limit;
void IcmpScanAttack_SendDialEvent(__be32 *pstr)
{
    DEVICE_EVENT* pstEvent = NULL;
    int ulSize = 0;
    char szip[16] = {0};
    if (log_limit & 2)
    {
    ...
    log_limit = log_limit | 2;
	snprintf(szip, 16, "%u.%u.%u.%u", NIPQUAD(*pstr));

    ulSize = sizeof(DEVICE_EVENT) + sizeof(szip);
    pstEvent = (DEVICE_EVENT*)kmalloc(ulSize + 1, GFP_ATOMIC);
    ...
    memset(pstEvent, 0, ulSize + 1);
    pstEvent->device_id                   = DEVICE_ID_SCANATTACK;
    pstEvent->event_code                  = 2;
    pstEvent->len                         = sizeof(szip);
    memcpy(pstEvent->data , szip, strlen(szip));


    device_event_report(pstEvent, ulSize + 1);
    kfree(pstEvent);
    return ;
}
#endif
	...
	if (rt->rt_flags & (RTCF_BROADCAST | RTCF_MULTICAST)) {
		...
		if ((icmph->type == ICMP_ECHO ||
            ...
            if (NULL != pszLogBuf)
            {
				...
				IcmpScanAttack_SendDialEvent(&saddr);
...
static const struct icmp_control icmp_pointers[NR_ICMP_TYPES + 1] = {
	...
	[ICMP_TIMESTAMP] = {
#ifdef CONFIG_ATP_ICMPTIMESTAP_DROP	
		.handler = icmp_discard,
#else
