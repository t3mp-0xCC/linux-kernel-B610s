...
extern void hw_rmnet_init_fastip(void);
    ...
    unsigned char *ea, struct net_device *rxdev)
{
    ...
    {
        br_conn_cache->txifp = (void *)vlan_dev_real_dev(rxdev);
        br_conn_cache->vid = vlan_dev_vlan_id(rxdev);
        br_conn_cache->action = ((vlan_dev_vlan_flags(rxdev) & 1) ?
...
bool fastip_is_ip_conn_cache_allowed(struct sk_buff *skb, u_int32_t hooknum)
{
    ...
    {
        FASTIP_TRACE("%s: kfastip_handle:%p not allow\n", __FUNCTION__ ,kfastip_handle);
        return FALSE;
    }
...
/*-------------------------------------------------
函数原型： bool fastip_nfct_help_pass(nf_conn_help *help)
描    述：        是否跳过help的连接记录
输    入： 	 nf_conn_help *help	
输    出：       无
返 回 值：   TRUE    passs
                          FALSE   failed
--------------------------------------------------*/
static bool fastip_nfct_help_pass(struct nf_conn_help *help)
{
    if(help && help->helper && help->helper->name)
    {
        FASTIP_TRACE("%s: help->name %s\n", __FUNCTION__,help->helper->name);
        if(((0 == strcmp(help->helper->name, "BCM-NAT"))||
            (0 == strcmp(help->helper->name, "ADDRCONE-NAT"))||
            (0 == strcmp(help->helper->name, "PORTCONE-NAT"))||
            (0 == strcmp(help->helper->name, "ALLCONE_NAT"))
            )
           )
        {
            return TRUE;
                    ...
                    struct nf_conntrack_tuple *manip)
{
    ...
    */
    ...
    {
        FASTIP_TRACE("%s: nfct_help help:%p hooknum:%x\n", __FUNCTION__,
            help, hooknum);
        /* return ; */        
        if(TRUE != fastip_nfct_help_pass(help))
        {
            return;
        }                                  
    }          
    ...
    */
    if(!strcmp(((struct net_device *)ip_conn_cache_entry.txif)->name,"br0"))
    {
        if (fastip_isbridge(kfastip_handle, ip_conn_cache_entry.txif)) 
        {
            ...
            FASTIP_TRACE("%s: fastip_br_conn_cache_lkup proto:%x pbr_conn_cache:%p\n", __FUNCTION__,
...
fastip_t *_fastip_attach(uint8 *name, uint32 *msg_level, fastip_detach_cb_t cb, void *arg)
{
    ...
    int32 size;
    fastip_osl_t *osh = (fastip_osl_t *)0xaabbccdd;
...
static int32 _fastip_dev_register(fastip_t *fastip_handle, void *dev, int32 isbridge, int32 enable)
{
    ...
    FASTIP_TRACE("%s:_fastip_dev_lkup dev:%s ctd:%p\n", __FUNCTION__, FASTIP_DEV_IFNAME(dev), ctd);
...
static int32 _fastip_forward(fastip_t *fastip_handle, void *p, void *rxifp)
{
    ...
    FASTIP_TRACE("%s: fastip_handle:%p p:%p rxifp:%p\n", __FUNCTION__, fastip_handle, p, rxifp);
    ...
    if (action & FASTIP_ACTION_TAG)
    {
        ...
        else
        {
            ...
            memcpy(data, 
                   data + FASTIP_VLAN_TAG_LEN, 
                   FASTIP_ETHER_ADDR_LEN);
            memcpy(data + FASTIP_ETHER_ADDR_LEN, 
                   data + FASTIP_VLAN_TAG_LEN + FASTIP_ETHER_ADDR_LEN, 
                   FASTIP_ETHER_ADDR_LEN);
            evh = (struct fastip_ethervlan_header *)data;
    ...
    else if (action & FASTIP_ACTION_UNTAG)
    {
        if (vid_in != -1)
        {
            ...
            memmove(data + FASTIP_ETHER_ADDR_LEN, 
                    data - FASTIP_VLAN_TAG_LEN + FASTIP_ETHER_ADDR_LEN, 
                    FASTIP_ETHER_ADDR_LEN);
            memmove(data, 
                    data - FASTIP_VLAN_TAG_LEN, 
...
static int32 __init fastip_module_init(void)
{
    ...
    printk("fastip ver:%s\n", FASTIP__VERSION);
    printk("%s: fastip_kattach kfastip_handle:%p\n", __FUNCTION__, kfastip_handle);
    fastip_attach_fn = _fastip_attach;
	
	hw_rmnet_init_fastip();
