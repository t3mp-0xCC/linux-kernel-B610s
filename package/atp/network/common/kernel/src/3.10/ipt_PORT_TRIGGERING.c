...
*/
	...
	DEBUGP("src ip :%u.%u.%u.%u,dst ip :%u.%u.%u.%u adfsadfasdfasfasfas\r\n",NIPQUAD(newrange.min_addr.ip),NIPQUAD(newrange.max_addr.ip));
...
*/    
static unsigned int
trigger_target(struct sk_buff *skb,const struct xt_action_param *xt_target_p)    
    
{
    //dump_stack();
    const struct ipt_trigger_info *info = xt_target_p->targinfo;
    const struct iphdr *iph = ip_hdr(skb);
    ...
    if (info->type == IPT_TRIGGER_OUT)
        return trigger_out(skb, xt_target_p->in, xt_target_p->out, xt_target_p->hooknum, xt_target_p->targinfo);
    else if (info->type == IPT_TRIGGER_IN)
        return trigger_in(skb, xt_target_p->in, xt_target_p->out, xt_target_p->hooknum, xt_target_p->targinfo);
    else if (info->type == IPT_TRIGGER_DNAT)
        return trigger_dnat(skb, xt_target_p->in, xt_target_p->out, xt_target_p->hooknum, xt_target_p->targinfo);
...
*/
    static int
trigger_check(const struct xt_tgchk_param *xt_tgchk_p)
{
    //dump_stack();
    //const struct ipt_trigger_info *info = targinfo;
    //const struct ipt_trigger_info *info = xt_tgchk_p->targinfo;
    struct list_head *cur_item, *tmp_item;
#if 0
    /*if ((strcmp(tablename, "mangle") == 0)) {
        DEBUGP("trigger_check: bad table `%s'.\n", tablename);*/
        printk("trigger_check:table =`%s'.\n", xt_tgchk_p->table);
    if ((strcmp(xt_tgchk_p->table, "mangle") == 0)) {
        DEBUGP("trigger_check: bad table `%s'.\n", xt_tgchk_p->table);
        return 0;
    }
    ...
    if (hook_mask & ~((1 << NF_INET_PRE_ROUTING) | (1 << NF_INET_FORWARD))) {
        ...
        printk("trigger_check: hooks %x.\n", xt_tgchk_p->hook_mask);
    if (xt_tgchk_p->hook_mask & ~((1 << NF_INET_PRE_ROUTING) | (1 << NF_INET_FORWARD))) {
        DEBUGP("trigger_check: bad hooks %x.\n", xt_tgchk_p->hook_mask); 
        return 0;
    }
    if (info->proto) {
