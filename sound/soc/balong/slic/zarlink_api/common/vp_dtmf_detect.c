    ...
    bool freshStart)
{
    ...
    if (freshStart) {
        ...
        pDtmfData->prevPrevHit = VP_DIG_NONE;
    }
}
    ...
    uint8 numSamples)
{
    ...
    while (i < numSamples) {
        ...
        if (hit != VP_DIG_NONE && hit == pDtmfData->prevHit) {
            pDtmfData->digitOutput = hit;
        }
        /* Look for 3 misses in a row to report the end of a digit */
        if (hit == VP_DIG_NONE && pDtmfData->prevHit == VP_DIG_NONE && pDtmfData->prevPrevHit == VP_DIG_NONE) {
            pDtmfData->digitOutput = VP_DIG_NONE;
        }
        pDtmfData->prevPrevHit = pDtmfData->prevHit;
