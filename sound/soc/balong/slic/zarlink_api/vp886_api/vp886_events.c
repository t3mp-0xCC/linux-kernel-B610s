 ...
 *
 * $Revision: 12031 $
 * $LastChangedDate: 2016-05-16 11:29:04 -0500 (Mon, 16 May 2016) $
 */
...
static void
Vp886DtmfHandleNewDigit(
    VpLineCtxType *pLineCtx,
    VpDigitType newDigit,
    uint16 timestampAdjust);
#endif
    ...
    VpLineCtxType *pLineCtx)
{
    ...
    if (enable && !pLineObj->dtmf.sampling) {
        ...
        #ifdef VP886_DTMF_ASYNC_SAMPLING
            pLineObj->dtmf.async.full = FALSE;
            pLineObj->dtmf.async.readIdx = 0;
            pLineObj->dtmf.async.writeIdx = 0;
            pLineObj->dtmf.async.majorOverflow = FALSE;
        #endif /* VP886_DTMF_ASYNC_SAMPLING */
...
#ifndef VP886_DTMF_ASYNC_SAMPLING
/** Vp886DtmfSampling()
    ...
    VpLineCtxType *pLineCtx)
{
    ...
    if (overflow != 0) {
        ...
        /* An overflow means we missed some samples. We have to reset the
           detection block before continuing because the new samples are not
           contiguous with the old ones. */
        VpDtmfDetectReset(pLineCtx, &pLineObj->dtmf.detectData, FALSE);
    }
    
    newDigit = VpDtmfDetectProcess(pLineCtx, pDtmfData, buffer, length);
    
    Vp886DtmfHandleNewDigit(pLineCtx, newDigit, 0);
...
#else /* VP886_DTMF_ASYNC_SAMPLING is defined */
    ...
    VpLineCtxType *pLineCtx)
{
    ...
    VpDigitType newDigit;
    uint16 numSamples;
    uint16 *pReadIdx = &pLineObj->dtmf.async.readIdx;
    uint16 timestampAdjust = 0;

    if (pLineObj->dtmf.async.full) {
        numSamples = VP886_DTMF_ASYNC_BUFFER_SIZE;
    } else {
        numSamples = ((pLineObj->dtmf.async.writeIdx + VP886_DTMF_ASYNC_BUFFER_SIZE) - *pReadIdx) % VP886_DTMF_ASYNC_BUFFER_SIZE;
    }
    
    if (pLineObj->dtmf.async.majorOverflow) {
        VP_WARNING(VpLineCtxType, pLineCtx, ("Vp886DtmfSampling async buffer overflow"));
    }

    while (numSamples > 0) {
        if (pLineObj->dtmf.async.overflows[*pReadIdx] == TRUE) {
            /* An overflow means we missed some samples. We have to reset the
               detection block before continuing because the new samples are not
               contiguous with the old ones. */
            VpDtmfDetectReset(pLineCtx, &pLineObj->dtmf.detectData, FALSE);
        ...
        /* Process one sample at a time to make it easier to look for the
           overflow flags at the same time */
        newDigit = VpDtmfDetectProcess(pLineCtx, pDtmfData, &pLineObj->dtmf.async.buf[*pReadIdx], 1);

        #if defined(VP886_DTMF_DETECT_4KHZ_SAMPLING)
            timestampAdjust = VpRoundedDivide(numSamples, 2);
        #elif defined(VP886_DTMF_DETECT_8KHZ_SAMPLING)
            timestampAdjust = VpRoundedDivide(numSamples, 4);
        #endif

        Vp886DtmfHandleNewDigit(pLineCtx, newDigit, timestampAdjust);

        numSamples--;
        *pReadIdx = (*pReadIdx + 1) % VP886_DTMF_ASYNC_BUFFER_SIZE;
        pLineObj->dtmf.async.full = FALSE;
    }
    ...
    uint16 timestampAdjust)
{
    ...
    if (newDigit != pLineObj->dtmf.currentDigit) {
        timestamp = Vp886GetTimestamp(pDevCtx);
        VP_DTMF(VpLineCtxType, pLineCtx, ("Vp886DtmfSampling: Digit change detected. New %d, Old %d.  ts %d, adjust %d",
            newDigit, pLineObj->dtmf.currentDigit, timestamp, timestampAdjust));

        if (pLineObj->dtmf.currentDigit != VP_DIG_NONE) {
            /* digit break event */
            if (newDigit == VP_DIG_NONE) {
                /* Move BREAK event timestamps back by 13.5ms because they
                   require 3 blocks of verification instead of the 2 that MAKE
                   events require.  Doing this keeps the digit and gap lengths
                   correct when looking at differences between timestamps. */
                timestampAdjust += 27;
            }
        ...
        if (newDigit != VP_DIG_NONE) {
            ...
            avgEnergy = (pDtmfData->peakEnergyRow + pDtmfData->peakEnergyCol) / 2;
            ...
            VP_DTMF(VpLineCtxType, pLineCtx, ("Digit level: row %lu, col %lu, avg %lu, sqrt %u, dBm %ld, output %d",
                pDtmfData->peakEnergyRow, pDtmfData->peakEnergyCol,
    ...
    VpLineCtxType *pLineCtx)
{
    ...
    uint8 n;
    uint8 i;
    uint16 readIdx = pLineObj->dtmf.async.readIdx;
    uint16 writeIdx = pLineObj->dtmf.async.writeIdx;
    int16 *pBuf = pLineObj->dtmf.async.buf;
    bool *pOverflows = pLineObj->dtmf.async.overflows;

    if (pLineCtx == VP_NULL) {
        VP_ERROR(None, VP_NULL, ("Vp886DtmfReadBuffer() received NULL pLineCtx"));
        return VP_STATUS_INVALID_ARG;
    }

    if (pLineCtx->pLineObj == VP_NULL) {
        VP_ERROR(VpLineCtxType, pLineCtx, ("Vp886DtmfReadBuffer() received NULL pLineCtx->pLineObj"));
        return VP_STATUS_INVALID_ARG;
    }

    /* If another function is in the middle of an MPI transaction,
       interrupting it here would corrupt it. */
    if (pDevObj->mpiBusy) {
        return VP_STATUS_SUCCESS;
    }

    /* Don't waste time reading the buffers if detection is disabled or the line
    ...
    VpMpiCmd(deviceId, ecVal, VP886_R_B1_RD, VP886_R_B1_LEN, regBuf);
    
    regOverflow = (regBuf[0] & VP886_R_B1_OVFL) >> 6;
    regPtr = regBuf[0] & VP886_R_B1_PTR;
    ...
    VpMemCpy(&regData[0], &regBuf[1], 24);
    
    /* Read more samples from the other buffer registers as necessary */
    if (regLength > 12) {
        VpMpiCmd(deviceId, ecVal, VP886_R_B2_RD, VP886_R_B2_LEN, regBuf);
        VpMemCpy(&regData[24], &regBuf[1], 24);
    }
    if (regLength > 24) {
        VpMpiCmd(deviceId, ecVal, VP886_R_B3_RD, VP886_R_B3_LEN, regBuf);
        VpMemCpy(&regData[48], &regBuf[1], 24);
    }
    if (regLength > 36) {
        VpMpiCmd(deviceId, ecVal, VP886_R_B4_RD, VP886_R_B4_LEN, regBuf);
        VpMemCpy(&regData[72], &regBuf[1], 24);
    }
    if (regLength > 48) {
        VpMpiCmd(deviceId, ecVal, VP886_R_B5_RD, VP886_R_B5_LEN, regBuf);
        VpMemCpy(&regData[96], &regBuf[1], 24);
    }

    if (regOverflow != 0) {
        /* Mark the first sample after an overflow, so we will know to reset
           the block when we process this */
        pOverflows[writeIdx] = TRUE;
    }

    /* Copy the raw data into the int16 buffer */
    for (n = 0; n < regLength; n++) {
        if (regOverflow != 0) {
            i = (n + regPtr) % 60;
        } else {
            i = n;
        ...
        /* Check for an overflow of the entire lineObj buffer */
        if (pLineObj->dtmf.async.full) {
            if (!pLineObj->dtmf.async.majorOverflow) {
                pLineObj->dtmf.async.majorOverflow = TRUE;
            }
            break;
        }

        /* Mark the first sample after a buffer overflow, so we will
           know to reset the detection block before we process it */
        if (n == 0 && regOverflow != 0) {
            /* Device buffer overflow occurred */
            pOverflows[writeIdx] = TRUE;
        } else if ((pLineObj->dtmf.async.majorOverflow)) {
            /* If we make it here, that means the buffer has been serviced (no
               longer full).  If there was previously a total buffer overflow,
               mark the first new sample as non-contiguous */
            pOverflows[writeIdx] = TRUE;
            pLineObj->dtmf.async.majorOverflow = FALSE;
        } else {
            pOverflows[writeIdx] = FALSE;
        }

        pBuf[writeIdx] = (int16)((regData[i*2] << 8) | regData[i*2+1]);

        writeIdx = (writeIdx + 1) % VP886_DTMF_ASYNC_BUFFER_SIZE;

        if (writeIdx == readIdx) {
            pLineObj->dtmf.async.full = TRUE;
        }
    }

    pLineObj->dtmf.async.writeIdx = writeIdx;
