...
extern int wlan_get_poweroff_status(void);
extern int wlan_set_poweroff_status(int poweroff_status); 
char apsta_sta_name[10] = "WiFi0";
#endif
...
#if defined(BSP_CONFIG_BOARD_E5)
        {
            if (!strncmp(ifr->ifr_name, apsta_sta_name, sizeof(apsta_sta_name)))
            {
                poweroff_status = wlan_get_poweroff_status();

                if (1 == poweroff_status) /* 1表示wifi关机已经置位， 不再响应业务命令*/
                {
                    printk("%s: poweroff_status = %d\n", __FUNCTION__, poweroff_status);
                    return -ENODEV;
                }
                else
                {
                    ret = standard(dev, iwr, cmd, info, handler);
                    return ret;
                }
            }
            else
