...
/*检测该地址是否是需要代理透传的地址*/
static int ipv6_chk_ndp_addr(struct inet6_dev *idev, const struct in6_addr *group)
{

    if ((idev)
        &&(idev->cnf.proxy_ndp)
        &&(group)
        &&(group->s6_addr32[0] == htonl(0xff020000))
        &&(group->s6_addr32[1] == htonl(0x00000000))
        &&(group->s6_addr32[2] == htonl(0x00000001))
        &&((group->s6_addr32[3] & htonl(0xFF000000)) == htonl(0xFF000000)))
    {
        return 1;
    }
			 ...
			 const struct in6_addr *src_addr)
{
	...
	if (idev) {
        ...
        else if (ipv6_chk_ndp_addr(idev, group))
        {
            /*在开启代理的情况下允许WAN侧的邻居请求通过*/
            rv = 1;
#endif
        }
		read_unlock_bh(&idev->lock);
