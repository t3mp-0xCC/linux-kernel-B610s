...
void UdpScanAttack_SendDialEvent(__be32 *pstr)
{
		   ...
		   int proto)
{
...
#ifdef CONFIG_ATP_ROUTE
		//socket bind with br+
        if (sk->sk_bound_in_fromlan == 1)
		{
			if ((skb->dev)
		       && (!IS_LAN_LO_DEVICE(skb->dev->name))) 
			{
				//printk("from wan \n");
				sock_put(sk);
			    goto drop;
    ...
    if (NULL != pszLogBuf)
    {
	    ...
	    if (NULL == strstr(pszLogBuf, ATP_LOOPBACK_ADDR_STR))
	    {
			...
			UdpScanAttack_SendDialEvent(&saddr);
	    }
